...
| **SWB / Welder** export page setup | ExcelJS, styled and bordered, but NO native page setup (portrait, no fit-to-width, no repeating heading row) — they do not call `xjPageSetup` | **Known gap, not a standard** (found 2026-09-26): RCD / IEL / TAT / Thermo / IRT / ELT were given print layout; SWB (board sheets + photos) and Welder (per-asset client-form sheets) were not. Candidate for a future pass; new modules must use `xjPageSetup` |
...
| **Older modules** | Home "Reset all results" uses the labelled underline button + an inline Yes / Cancel confirm row built from home-grown state (the LOOK is the standard; new code builds it with `ConfirmReset` instead); module-local copies of the editable dropdown / style objects (`SI`, `ST`, `SS`, `STH`); TAT's site delete leaves meta/history behind; RCD/IEL read an "audit entered" flag straight from `localStorage` | Legacy — **new modules use the shared pieces**; unifying is a future pass |
