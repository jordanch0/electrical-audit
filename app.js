...
const cC = header.findIndex(h => h.includes("circuit") || h.includes("cb"));
...
if (hasSeparateCircuit) {
  // New 3-column format: Area | Panel | Circuit
  const cRaw = String(row[cC] || "").trim();
  if (pRaw || cRaw) rawByArea[aKey].push({ panel: pRaw, circuit: cRaw || pRaw });
} else {
...
function downloadTATTemplate(){
  ...
  const rows=[
    ...
    ["Workshop","TAG-002","4.5\" Angle Grinder","Power Tool","3"],
    ["Workshop","TAG-003","10m Extension Lead","Extension Lead","6"],
    ["Workshop","TAG-004","20m Extension Lead","Extension Lead","6"],
    ["Workshop","TAG-005","Bench Drill Press","Power Tool","6"],
    ["Office","TAG-006","Laptop Charger — Dell","Appliance","12"],
    ["Office","TAG-007","Monitor Power Supply","Appliance","12"],
    ["Smoko Hut","TAG-008","Microwave","Appliance","12"],
    ["Smoko Hut","TAG-009","Kettle","Appliance","12"],
    ["Wash Plant","TAG-010","Pressure Washer","Power Tool","3"],
  ];
  const ws=XLSX.utils.aoa_to_sheet(rows);
...
function SWBItemPage({itemKey,board,area,project,results,dropdowns,onPatch,onClose}) {
  ...
  const doSave=()=>{const isFail=status===SWB_STATUS.FAIL;onPatch(itemKey,{status,defectId,comment,risk,rectified:isFail?rectified:"",responsibility:isFail?resp:"",priority:isFail?priority:""});onClose();};
