...
function exportIELExcel(project, results, meta) {
  ...
  rows.push([`Vorick Group Asset Maintenance Pty. Ltd.  |  ABN: 44 601 045 872  |  Electrical Licence: 319114C`,...Array(n-1).fill("")]);
...
function parseIELExcel(data){
  ...
  return{siteName,siteId,areas,results};
}
...
function downloadIELTemplate(){
  ...
  const rows=[
    ...
    ["ONR Wash Plant","Estop","Screen Conveyor 1","","","","","","","","","","",""],
    ["ONR Wash Plant","Lanyard","Coarse Conveyor 1","","","","","","","","","","",""],
    ["ONR Wash Plant","Isolator","Primary Crusher DR21","","","","","","","","","","",""],
  ];
  const ws=XLSX.utils.aoa_to_sheet(rows);
...
function IELProjectListView({projects,allResults,onSelect,onAddProject,onDeleteProject}){
  ...
  const confirmImport=()=>{
    ...
    const proj={id:importPreview.siteId,name:sName,areas:importPreview.areas};
      ...
      IEL_CATEGORIES.forEach(cat=>{
        proj.areas.forEach(a=>a.panels.forEach(p=>{
          if(p.name===cat.key)p.circuits.forEach(id=>{
            total++;if(ielGetStatus(allResults,proj.id,a.id,cat.key,id)===IEL_STATUS.FAIL)fails++;
          });
        }));
      });
      const pct=total>0?Math.round(((total-fails)/total)*100):0;
      return React.createElement('div',{key:proj.id,style:{...SI.siteCard,flexDirection:"column",gap:0,padding:0,overflow:"hidden"}}
        ,React.createElement('button',{style:{display:"flex",justifyContent:"space-between",alignItems:"center",width:"100%",background:"transparent",border:"none",cursor:"pointer",padding:"16px 18px",color:"inherit",textAlign:"left"},onClick:()=>onSelect(proj.id)}
          ,React.createElement('div',{style:{flex:1}}
            ...
            ,React.createElement('div',{style:SI.siteCardSub},proj.areas.length," areas · ",total," items · 3-month cycle")
            ,React.createElement('div',{style:{width:"100%",height:4,background:"#2a2a2a",borderRadius:2,marginTop:8,overflow:"hidden"}}
              ,React.createElement('div',{style:{height:"100%",borderRadius:2,width:`${pct}%`,background:fails>0?"#ef4444":"#10b981",transition:"width 0.4s"}})
            )
          )
          ,React.createElement('div',{style:{display:"flex",alignItems:"center",gap:8,marginLeft:16}}
      ...
      ?React.createElement('div',{style:SI.addCard}
        ...
        ,tab==="manual"&&React.createElement(React.Fragment,null
          ...
          ,React.createElement('div',{style:{marginBottom:10}}
            ...
            ,React.createElement('input',{style:{...SI.metaInput,marginTop:4},value:newName,placeholder:"e.g. Dixon Quarries — ONR Wash Plant",onChange:e=>setNewName(e.target.value)})
          )
          ,React.createElement('div',{style:{display:"flex",gap:8}}
            ,React.createElement('button',{style:SI.ctaPrimary,onClick:()=>{if(!newName.trim())return;onAddProject({id:ielSlug(newName),name:newName.trim(),areas:[]},{});setNewName("");setShowAdd(false);}},"Add Site")
      ...
      ,(item.status===IEL_STATUS.FAIL)&&React.createElement(React.Fragment,null
        ...
        ,React.createElement('div',{style:{display:"flex",gap:10}}
          ,React.createElement('div',{style:{...SI.modalField,flex:2}}
            ...
            ,React.createElement('input',{style:SI.modalInput,value:item.responsibility||"",placeholder:"e.g. Vorick Group",onChange:e=>onPatch({responsibility:e.target.value})})
          )
    ...
    ,React.createElement('div',{style:{background:"#161616",border:"1px solid #2a2a2a",borderRadius:12,padding:"12px 14px",marginBottom:14}}
      ,editingProject
        ?React.createElement(React.Fragment,null
          ...
          ,React.createElement('div',{style:{display:"flex",gap:8}}
            ,React.createElement('button',{style:SI.ctaPrimary,onClick:()=>{upd({...project,name:projName.trim()||project.name});setEditingProject(false);}},"Save")
...
function CalendarApp({ onGoHome }) {
  ...
  React.useEffect(()=>{
    (async()=>{
      try{
        const [ev, rcdProjs, ielProjs] = await Promise.all([
          load(K_CAL_EVENTS,[]),
          load(K_PROJECTS,[]),
          load(K_IEL_PROJECTS,[]),
        ]);
  ...
  const saveEvent = () => {
    if(!form.dueDate||!form.site.trim()||form.site==="__custom__") return;
    if(editId){
      ...
      if(form.recur==="none"){
        newEvents.push({id:calUid(),...form,seriesId:null});
      } else {
  ...
  const EventCard = ({ev, compact=false, onToggleComplete}) => {
    ...
    return React.createElement('div',{style:{background:"#1a1a1a",border:`2px solid ${uc}44`,borderRadius:12,padding:"12px 14px",marginBottom:8,opacity:ev.completed?0.7:1}}
      ,React.createElement('div',{style:{display:"flex",alignItems:"flex-start",gap:10}}
        ,React.createElement('button',{
          ...
          onClick:()=>(onToggleComplete||toggleComplete)(ev.id),
        ...
        ,!compact&&ev.completed&&React.createElement('button',{style:{...SI.smallBtn,color:"#555",marginLeft:8},onClick:()=>toggleComplete(ev.id)},"↩ Undo")
      )
    );
  };
  ...
  return React.createElement('div',{style:CAL_STYLE.root}
    ...
    ,React.createElement('main',{style:CAL_STYLE.main}
      ,view==="upcoming"&&!showAdd&&React.createElement('div',{style:{padding:"16px"}}
        // Alert banner if overdue
        ,overdue.length>0&&React.createElement('div',{style:{background:"#3d1a1a",border:"2px solid #ef4444",borderRadius:12,padding:"12px 16px",marginBottom:16}}
          ,React.createElement('div',{style:{fontSize:13,fontWeight:800,color:"#f87171",marginBottom:8}},"⚠ ",overdue.length," OVERDUE TEST",overdue.length!==1?"S":"")
          ,overdue.map(ev=>React.createElement(EventCard,{key:ev.id,ev,compact:false}))
        )
        // Due this week
        ,upcoming7.length>0&&React.createElement('div',{style:{marginBottom:16}}
          ,React.createElement('div',{style:{fontSize:11,color:"#ef4444",fontWeight:700,letterSpacing:1,marginBottom:8}},"⚡ DUE THIS WEEK")
          ,upcoming7.map(ev=>React.createElement(EventCard,{key:ev.id,ev}))
        )
        // Due this month
        ,upcoming30.length>0&&React.createElement('div',{style:{marginBottom:16}}
          ,React.createElement('div',{style:{fontSize:11,color:"#f59e0b",fontWeight:700,letterSpacing:1,marginBottom:8}},"📅 DUE THIS MONTH")
          ,upcoming30.map(ev=>React.createElement(EventCard,{key:ev.id,ev}))
        )
        // Future
        ,future.length>0&&React.createElement('div',{style:{marginBottom:16}}
          ,React.createElement('div',{style:{fontSize:11,color:"#555",fontWeight:700,letterSpacing:1,marginBottom:8}},"🗓 UPCOMING")
          ,future.map(ev=>React.createElement(EventCard,{key:ev.id,ev}))
        )
        ,activeEvents.length===0&&completedEvents.length===0&&React.createElement('div',{style:{textAlign:"center",padding:"40px 20px",color:"#555"}}
          ...
          ,React.createElement('div',{style:{fontSize:13,color:"#444",marginBottom:20}},"Tap + Add Event to schedule your first test due date")
        )
        ,completedEvents.length>0&&React.createElement('div',{style:{marginBottom:16}}
          ,React.createElement('div',{style:{fontSize:11,color:"#22c55e",fontWeight:700,letterSpacing:1,marginBottom:8}},"✓ COMPLETED (",completedEvents.length,")")
          ,completedEvents.map(ev=>React.createElement(EventCard,{key:ev.id,ev,onToggleComplete:toggleComplete}))
        )
      )
          ...
          ,allSites.length>0
            ?React.createElement(React.Fragment,null
              ,React.createElement('select',{
                style:{width:"100%",background:"#1a1a1a",border:`1px solid ${!form.site?"#ef4444":"#333"}`,borderRadius:8,color:form.site?"#eee":"#666",padding:"10px 12px",fontSize:13,outline:"none",boxSizing:"border-box",marginBottom:6},
                value:form.site,
              ...
              ,form.site==="__custom__"&&React.createElement('input',{
                ...
                autoFocus:true,
                onChange:e=>setForm(f=>({...f,site:e.target.value==="__custom__"?"":e.target.value}))
              })
            )
            :React.createElement('input',{style:{width:"100%",background:"#1a1a1a",border:`1px solid ${!form.site?"#ef4444":"#333"}`,borderRadius:8,color:"#eee",padding:"10px 12px",fontSize:13,outline:"none",boxSizing:"border-box"},value:form.site,placeholder:"e.g. Dixon Quarries — ONR Wash Plant",onChange:e=>setForm(f=>({...f,site:e.target.value}))})
