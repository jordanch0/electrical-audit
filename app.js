...
function IELApp({ onGoHome }) {
  ...
  React.useEffect(()=>{
    ...
    (async()=>{
      try{
        const[p,r,m,h,cat]=await Promise.all([load(K_IEL_PROJECTS,[]),load(K_IEL_RESULTS,{}),load(K_IEL_META,{}),load(K_IEL_HISTORY,[]),load(K_IEL_CAT,null)]);
        setActiveCat(cat);
...
function SWBAuditGate({summary, hasAuditor, onGoHome, onEnterAudit}) {
  ...
  return React.createElement('div',{style:{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px",textAlign:"center",minHeight:"60vh",gap:0}}
    ...
    ,auditInProgress&&React.createElement('button',{style:{padding:"14px 32px",background:"#a855f7",color:"#fff",border:"none",borderRadius:14,fontSize:15,fontWeight:800,cursor:"pointer",marginBottom:12,width:"100%",maxWidth:320},onClick:onEnterAudit},"☑ Continue Audit →")
    ...
    ,React.createElement('header',{style:{...SS.topbar,borderBottom:`2px solid ${view==="projects"?"#222":"#a855f733"}`}}
      ,React.createElement('div',{style:SS.topbarLeft}
        ,view!=="projects"&&React.createElement('button',{style:SS.backBtn,onClick:()=>{
          ...
          else if(view==="audit"&&auditEntered&&!activeAreaId){setAuditEntered(false);setView("audit");}
  ...
  return React.createElement('div',{style:SS.homeWrap}
    ...
    ,React.createElement('div',{style:{width:"100%",maxWidth:500,background:"#161616",border:`1px solid ${auditInProgress?"#a855f744":"#2a2a2a"}`,borderRadius:12,padding:"10px 14px"}}
      ...
      ,archiveMsg
        ...
        :auditInProgress
          ?React.createElement(SWBCompleteAuditBtn,{onComplete:()=>{onArchive();setArchiveMsg("📁 Archived — reset for new audit");setTimeout(()=>setArchiveMsg(""),3000);}})
