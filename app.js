...
, React.createElement('button', {
      ...
      , React.createElement('button', {
        ...
        , React.createElement('div', {style:{flex:1}}
          ...
          , React.createElement('div', {style:{fontSize:12,color:"#888",marginTop:4,lineHeight:1.5}}, "Visual inspection of switchboards and enclosures — annual audit with defect tracking")
          , React.createElement('div', {style:{display:"flex",gap:8,marginTop:8,flexWrap:"wrap"}}
...
const SWB_RISK_COLORS = { L:"#22c55e", M:"#f59e0b", H:"#ef4444", U:"#8B0000" };
...
function swbAddYears(d,n){ if(!d)return""; try{const x=new Date(d);x.setFullYear(x.getFullYear()+n);return x.toLocaleDateString("en-AU",{day:"2-digit",month:"2-digit",year:"numeric"});}catch(_){return"";} }
function swbFmtDate(d){ if(!d)return""; try{return new Date(d).toLocaleDateString("en-AU",{day:"2-digit",month:"2-digit",year:"numeric"});}catch(_){return d;} }

// ─── Data helpers ──────────────────────────────────────────────────────────
function swbGetItem(results, pid, aid, bid, key) {
  return ((((results[pid]||{})[aid]||{})[bid])||{})[key]
    || {status:"untested",defectId:"",comment:"",risk:""};
}
function swbGetStatus(results, pid, aid, bid, key) {
  return swbGetItem(results,pid,aid,bid,key).status||"untested";
}
function swbBoardSummary(results, pid, aid, bid) {
  let pass=0,fail=0,na=0,untested=0;
  SWB_CHECKLIST.forEach(({key})=>{
    const v=swbGetStatus(results,pid,aid,bid,key);
    if(v==="pass")pass++; else if(v==="fail")fail++; else if(v==="na")na++; else untested++;
  });
...
function swbSiteSummary(results, project) {
  ...
  const total=(project.areas||[]).reduce((s,a)=>s+(a.boards||[]).length*SWB_CHECKLIST.length,0);
  return {pass,fail,na,untested,total};
}

// ─── Excel export ──────────────────────────────────────────────────────────
const SWB_XC = {
  purple:"FFA855F7", white:"FFFFFFFF", darkGrey:"FF2D2D2D",
  lightGrey:"FFF5F5F5", midGrey:"FFD9D9D9", boardHdr:"FF1E1E2E",
  priorityU_bg:"FF9B0000",priorityU_font:"FFFFFFFF",
  priorityH_bg:"FFFFC7CE",priorityH_font:"FF9C0006",
  priorityM_bg:"FFFFD966",priorityM_font:"FF7F6000",
  priorityL_bg:"FFE2EFDA",priorityL_font:"FF375623",
};
function swbXBorder(s="thin",c=SWB_XC.midGrey){return{style:s,color:{rgb:c}};}
function swbXAllBorders(){const b=swbXBorder();return{top:b,bottom:b,left:b,right:b};}
function swbXBtm(){return{bottom:swbXBorder("hair")};}
function swbXCS(fill,font={},align={},borders={}){
  return{fill:{patternType:"solid",fgColor:{rgb:fill}},font:{name:"Calibri",sz:10,...font},alignment:{vertical:"center",...align},border:borders};
}
function swbXPriColor(p){
  if(p==="U")return{bg:SWB_XC.priorityU_bg,font:SWB_XC.priorityU_font,bold:true};
  if(p==="H")return{bg:SWB_XC.priorityH_bg,font:SWB_XC.priorityH_font,bold:false};
  if(p==="M")return{bg:SWB_XC.priorityM_bg,font:SWB_XC.priorityM_font,bold:false};
  if(p==="L")return{bg:SWB_XC.priorityL_bg,font:SWB_XC.priorityL_font,bold:false};
  return null;
}
function swbXRowStyle(ri,risk){
  if(risk){const pc=swbXPriColor(risk);if(pc)return swbXCS(pc.bg,{sz:10,color:{rgb:pc.font},bold:pc.bold},{wrapText:true},swbXBtm());}
  const bg=ri%2===0?SWB_XC.white:SWB_XC.lightGrey;
  return swbXCS(bg,{sz:10,color:{rgb:SWB_XC.darkGrey}},{wrapText:true},swbXBtm());
}
function swbXSetCell(ws,ref,value,style){ws[ref]={v:value!=null?value:"",t:typeof value==="number"?"n":"s",s:style};}

function exportSWBExcel(project,results,meta){
  const wb=XLSX.utils.book_new();
  const ws={};
  const n=6; const cols="ABCDEF".split("");
  const range={s:{r:0,c:0},e:{r:0,c:n-1}};
  ws["!merges"]=[];
  let r=0;

  const sName=project.name||"Site";
  const testDate=(meta&&meta.testDate)||"";
  const nextDue=swbAddYears(testDate,1);
  ...
  const titleSt   =swbXCS(SWB_XC.darkGrey,{bold:true,sz:14,color:{rgb:SWB_XC.white}},{horizontal:"left"});
  const subtitleSt=swbXCS("FF1E1E1E",{sz:9,color:{rgb:"FFbbbbbb"}},{horizontal:"left"});
  const metaSt    =swbXCS("FF262626",{sz:9,color:{rgb:"FF999999"}},{horizontal:"left"});
  const spacerSt  =swbXCS("FF1E1E1E");
  const hdrSt     =swbXCS(SWB_XC.purple,{bold:true,sz:10,color:{rgb:SWB_XC.white}},{horizontal:"center",wrapText:true},swbXAllBorders());
  const boardHdrSt=swbXCS(SWB_XC.boardHdr,{bold:true,sz:11,color:{rgb:"FFc084fc"}},{horizontal:"left"},{bottom:swbXBorder("medium","FFa855f7")});
  const photoRowSt=swbXCS("FF181820",{sz:9,color:{rgb:"FF444466"},italic:true},{horizontal:"left"});

  // Header rows
  swbXSetCell(ws,"A1",`${sName}  –  Switchboard / Enclosure Audit`,titleSt);
  for(let c=1;c<n;c++)swbXSetCell(ws,cols[c]+"1","",swbXCS(SWB_XC.darkGrey));
  ws["!merges"].push({s:{r:0,c:0},e:{r:0,c:n-1}}); r=1;

  swbXSetCell(ws,"A2",coLine,subtitleSt);
  for(let c=1;c<n;c++)swbXSetCell(ws,cols[c]+"2","",swbXCS("FF1E1E1E"));
  ws["!merges"].push({s:{r:1,c:0},e:{r:1,c:n-1}}); r=2;

  swbXSetCell(ws,"A3",`Auditor: ${(meta&&meta.auditor)||""}`,metaSt);
  swbXSetCell(ws,"B3","",metaSt);
  swbXSetCell(ws,"C3",`Date Tested: ${swbFmtDate(testDate)}`,metaSt);
  swbXSetCell(ws,"D3","",metaSt);
  swbXSetCell(ws,"E3",`Next Annual Audit Due: ${nextDue}`,metaSt);
  swbXSetCell(ws,"F3","",metaSt);
  ws["!merges"].push({s:{r:2,c:0},e:{r:2,c:1}});
  ws["!merges"].push({s:{r:2,c:2},e:{r:2,c:3}});
  ws["!merges"].push({s:{r:2,c:4},e:{r:2,c:5}}); r=3;

  for(let c=0;c<n;c++)swbXSetCell(ws,cols[c]+"4","",spacerSt);
  ws["!merges"].push({s:{r:3,c:0},e:{r:3,c:n-1}}); r=4;

  const headers=["Item","Pass / Fail","Defect ID","Comments","Risk","Area / Board"];
  headers.forEach((h,i)=>swbXSetCell(ws,cols[i]+"5",h,hdrSt)); r=5;

  let rowIdx=0;
  (project.areas||[]).forEach(area=>{
    (area.boards||[]).forEach(board=>{
      const boardLabel=`${area.name}  ›  ${board.name}`;
      swbXSetCell(ws,cols[0]+(r+1),boardLabel,boardHdrSt);
      for(let c=1;c<n;c++)swbXSetCell(ws,cols[c]+(r+1),"",boardHdrSt);
      ws["!merges"].push({s:{r,c:0},e:{r,c:n-1}}); r++;

      SWB_CHECKLIST.forEach(({key,label})=>{
        const item=(((results[area.id]||{})[board.id])||{})[key]||{status:"untested",defectId:"",comment:"",risk:""};
        const st=item.status||"untested";
        const pfLabel=st==="pass"?"Pass":st==="fail"?"Fail":st==="na"?"N/A":"Untested";
        const rs=swbXRowStyle(rowIdx,item.risk||"");
        swbXSetCell(ws,cols[0]+(r+1),label,rs);
        swbXSetCell(ws,cols[1]+(r+1),pfLabel,swbXCS(rs.fill.fgColor.rgb,{sz:10,color:rs.font.color,bold:rs.font.bold},{horizontal:"center",wrapText:true},swbXBtm()));
        swbXSetCell(ws,cols[2]+(r+1),item.defectId||"",rs);
        swbXSetCell(ws,cols[3]+(r+1),item.comment||"",rs);
        swbXSetCell(ws,cols[4]+(r+1),item.risk||"",swbXCS(rs.fill.fgColor.rgb,{sz:10,color:rs.font.color,bold:rs.font.bold},{horizontal:"center"},swbXBtm()));
        swbXSetCell(ws,cols[5]+(r+1),boardLabel,rs);
        r++; rowIdx++;
      });

      // Photo spacer row
      swbXSetCell(ws,cols[0]+(r+1),"← Insert photos here",photoRowSt);
      for(let c=1;c<n;c++)swbXSetCell(ws,cols[c]+(r+1),"",swbXCS("FF181820"));
      ws["!merges"].push({s:{r,c:0},e:{r,c:n-1}}); r++;
    });
  });
  ...
  ws["!cols"]=[{wch:30},{wch:14},{wch:12},{wch:48},{wch:8},{wch:28}];
  ws["!rows"]=[];for(let i=0;i<=r;i++)ws["!rows"].push({hpt:18});
  ...
  if(window.webkit&&window.webkit.messageHandlers&&window.webkit.messageHandlers.shareFile){
    window.webkit.messageHandlers.shareFile.postMessage({base64:wbOut,filename:fname});
  } else {
...
function downloadSWBTemplate(){
  ...
  XLSX.utils.book_append_sheet(wb,ws,"Switchboard List");
...
function parseSWBExcel(data){
  ...
  for(let i=0;i<Math.min(rows.length,10);i++){
    const r=rows[i].map(c=>String(c).toLowerCase());
    if(r.some(c=>c.includes("area"))||r.some(c=>c.includes("board")||c.includes("panel"))){hdrIdx=i;break;}
  }
  const hdr=rows[hdrIdx].map(c=>String(c).toLowerCase().trim());
  const aC=Math.max(0,hdr.findIndex(h=>h.includes("area")));
  let bC=hdr.findIndex(h=>h.includes("board")||h.includes("panel")||h.includes("name"));
  if(bC<0)bC=aC+1;

  // Extract company/abn/licence from top rows
  let company="",abn="",licence="";
  for(let i=0;i<hdrIdx;i++){
    const rowStr=rows[i].map(c=>String(c)).join(" ");
    if(!company){const m=rowStr.match(/company[:\s]+([^|]+)/i);if(m)company=m[1].trim();}
    if(!abn){const m=rowStr.match(/abn[:\s]+([0-9 ]+)/i);if(m)abn=m[1].trim();}
    if(!licence){const m=rowStr.match(/licen[cs]e[:\s]+([^\s|,]+)/i);if(m)licence=m[1].trim();}
  }
  ...
  const siteId=swbSlug("imported");
  const areas=Object.entries(areaMap).map(([aName,boards])=>({
    id:swbSlug(aName),
    name:aName,
    boards:[...boards].map(bName=>({id:swbSlug(bName),name:bName})),
  }));
...
function SWBApp({onGoHome}){
  ...
  React.useEffect(()=>{
    (async()=>{
      try{
        const[p,r,m,h]=await Promise.all([load(K_SWB_PROJECTS,[]),load(K_SWB_RESULTS,{}),load(K_SWB_META,{}),load(K_SWB_HISTORY,[])]);
        setProjects(p);setAllResults(r);setAllMeta(m);setHistory(h);
  ...
  const patchItem=(aid,bid,key,patch)=>{
    setAllResults(prev=>{
      const site=prev[activeProject]||{};
      const ar=site[aid]||{};
      const bd=ar[bid]||{};
      const old=bd[key]||{status:"untested",defectId:"",comment:"",risk:""};
      return{...prev,[activeProject]:{...site,[aid]:{...ar,[bid]:{...bd,[key]:{...old,...patch}}}}};
    });
  };
  const cycleItem=(aid,bid,key)=>{
    const cur=swbGetStatus(allResults,activeProject,aid,bid,key);
    const next=cur==="untested"?"pass":cur==="pass"?"fail":cur==="fail"?"na":"untested";
    patchItem(aid,bid,key,{status:next});
  };
  const archiveAudit=()=>{
    const snap={id:swbUid(),projectId:activeProject,projectName:project?.name||"",testDate:meta.testDate||"",auditor:meta.auditor||"",archivedAt:new Date().toISOString(),results:JSON.parse(JSON.stringify(allResults[activeProject]||{})),meta:{...meta}};
    setHistory(prev=>[snap,...prev].slice(0,100));
    return snap;
  };
  ...
  const goAudit=()=>{setView("audit");setActiveBoardId(null);setActiveItemKey(null);};

  if(!loaded)return React.createElement('div',{style:{display:"flex",flexDirection:"column",flex:1,alignItems:"center",justifyContent:"center",background:"#111"}},
    React.createElement('div',{style:{width:36,height:36,border:"3px solid #333",borderTop:`3px solid ${SWB_COLOR}`,borderRadius:"50%",animation:"spin 0.8s linear infinite"}})
  );

  const SS=swbStyles();
  const summary=project?swbSiteSummary(allResults,project):{total:0,pass:0,fail:0,na:0,untested:0};
  const isAudit=view==="audit"||view==="board";

  return React.createElement('div',{style:SS.root}
    // Top bar
    ,React.createElement('header',{style:{...SS.topbar,borderBottom:`2px solid ${SWB_COLOR}33`}}
      ,React.createElement('div',{style:SS.topbarLeft}
        ,view!=="projects"&&React.createElement('button',{style:SS.backBtn,onClick:()=>{
          if(view==="board"){goAudit();}
          else if(view==="audit"){goHome();}
        ...
        ,React.createElement('div',null
          ...
          ,React.createElement('div',{style:{...SS.appSub,color:view==="projects"?"#555":SWB_COLOR}},
            view==="projects"?"Project Select":project?.name||"")
        )
      )
      ,React.createElement('div',{style:SS.topbarRight}
        ...
        ,React.createElement('button',{style:{...SS.smallBtn,color:"#aaa",borderColor:"#333",fontSize:11,padding:"5px 10px"},onClick:onGoHome},"⌂ Modules")
      )
    )
    // Breadcrumb
    ,view!=="projects"&&React.createElement('div',{style:SS.breadcrumb}
      ,React.createElement('span',{style:{...SS.bcItem,color:SWB_COLOR,cursor:"pointer"},onClick:goHome},project?.name||"")
      ,view==="audit"&&React.createElement(React.Fragment,null
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:SWB_COLOR}},"Audit")
      )
      ,view==="board"&&board&&React.createElement(React.Fragment,null
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:"#aaa",cursor:"pointer"},onClick:goAudit},area?.name||"")
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:SWB_COLOR}},board.name)
      )
      ,view==="manage"&&React.createElement(React.Fragment,null,React.createElement('span',{style:SS.bcSep},"›"),React.createElement('span',{style:{...SS.bcItem,color:"#a855f7"}},"Manage"))
      ,view==="history"&&React.createElement(React.Fragment,null,React.createElement('span',{style:SS.bcSep},"›"),React.createElement('span',{style:{...SS.bcItem,color:"#f59e0b"}},"History"))
      ,view==="report"&&React.createElement(React.Fragment,null,React.createElement('span',{style:SS.bcSep},"›"),React.createElement('span',{style:{...SS.bcItem,color:SWB_COLOR}},"Report"))
    )
    // Main
    ,React.createElement('div',{style:SS.main}
      ,view==="projects"&&React.createElement(SWBProjectListView,{
        projects,allResults,
        onSelect:pid=>{setActiveProject(pid);setView("home");},
        onAddProject:(p,r)=>{setProjects(prev=>[...prev,p]);if(r)setAllResults(prev=>({...prev,[p.id]:r}));},
        onDeleteProject:pid=>{setProjects(prev=>prev.filter(p=>p.id!==pid));setAllResults(prev=>{const n={...prev};delete n[pid];return n;});setAllMeta(prev=>{const n={...prev};delete n[pid];return n;});},
      })
      ,view==="home"&&project&&React.createElement(SWBProjectHomeView,{
        project,meta,setMeta,results:allResults,
        onStartAudit:()=>setView("audit"),
        onReport:()=>setView("report"),
        onManage:()=>setView("manage"),
        onHistory:()=>setView("history"),
        onExport:()=>exportSWBExcel(project,allResults[activeProject]||{},meta),
        onArchive:archiveAudit,
        summary,
      })
      ,view==="audit"&&project&&React.createElement(SWBAuditListView,{
        project,results:allResults,pid:activeProject,
        onSelectBoard:(aid,bid)=>{setActiveAreaId(aid);setActiveBoardId(bid);setView("board");},
      })
      ,view==="board"&&board&&React.createElement(SWBBoardView,{
        board,area,project,pid:activeProject,results:allResults,activeItemKey,
        onCycle:key=>cycleItem(activeAreaId,activeBoardId,key),
        onOpenItem:key=>setActiveItemKey(key),
        onCloseItem:()=>setActiveItemKey(null),
        onPatch:(key,patch)=>patchItem(activeAreaId,activeBoardId,key,patch),
      })
      ,view==="report"&&project&&React.createElement(SWBReportView,{
        project,results:allResults,pid:activeProject,meta,
        onExport:()=>exportSWBExcel(project,allResults[activeProject]||{},meta),
        onArchive:archiveAudit,
      })
      ,view==="manage"&&project&&React.createElement(SWBManageView,{
        project,
        onUpdateProject:updated=>setProjects(prev=>prev.map(p=>p.id===updated.id?updated:p)),
      })
      ,view==="history"&&React.createElement(SWBHistoryView,{
        history:history.filter(h=>h.projectId===activeProject),
        onDelete:id=>setHistory(prev=>prev.filter(h=>h.id!==id)),
      })
    )
    ...
    ,view!=="projects"&&React.createElement('nav',{style:SS.bottomNav}
      ,React.createElement(SWBNavBtn,{icon:"⌂",label:"Home",   active:view==="home",  onClick:goHome,          color:SWB_COLOR})
      ,React.createElement(SWBNavBtn,{icon:"☑",label:"Audit",  active:isAudit,        onClick:()=>setView("audit"), color:SWB_COLOR})
      ,React.createElement(SWBNavBtn,{icon:"≡",label:"Report", active:view==="report",onClick:()=>setView("report"),color:SWB_COLOR})
...
function SWBProjectListView({projects,allResults,onSelect,onAddProject,onDeleteProject}){
  ...
  const handleFile=e=>{
    ...
    reader.onload=ev=>{
      try{
        const data=XLSX.read(ev.target.result,{type:"array"});
        const parsed=parseSWBExcel(data);
        setImportName(parsed.siteName||file.name.replace(/\.(xlsx|xls|csv)$/i,"").replace(/[_-]+/g," ").trim());
        setImportPreview(parsed);
        if(!importCo&&parsed.company)setImportCo(parsed.company);
        if(!importAbn&&parsed.abn)setImportAbn(parsed.abn);
  ...
  return React.createElement('div',{style:SS.listWrap}
    ,React.createElement('div',{style:SS.brandBlock}
      ,React.createElement('div',{style:SS.brandTitle},"VORICK GROUP")
      ,React.createElement('div',{style:SS.brandSub},"Switchboard Audit Management")
    )
    ,React.createElement('div',{style:{...SS.listTitle,marginTop:24}},"Sites")
    ,projects.length===0&&React.createElement('div',{style:{color:"#555",fontSize:14,marginBottom:16}},"No sites yet — add one or import from Excel.")
    ,projects.map(proj=>{
      const s=swbSiteSummary(allResults,proj);
      const totalBoards=(proj.areas||[]).reduce((n,a)=>n+(a.boards||[]).length,0);
      const pct=s.total>0?Math.round((s.pass+s.na)/s.total*100):0;
      return React.createElement('div',{key:proj.id,style:{...SS.siteCard,flexDirection:"column",gap:0,padding:0,overflow:"hidden"}}
        ,React.createElement('button',{style:{display:"flex",justifyContent:"space-between",alignItems:"center",width:"100%",background:"transparent",border:"none",cursor:"pointer",padding:"16px 18px",color:"inherit",textAlign:"left"},onClick:()=>onSelect(proj.id)}
          ,React.createElement('div',{style:{flex:1}}
            ...
            ,React.createElement('div',{style:SS.siteCardSub},(proj.company?proj.company+" · ":""),proj.areas.length," areas · ",totalBoards," boards")
            ,React.createElement('div',{style:{width:"100%",height:4,background:"#2a2a2a",borderRadius:2,marginTop:8,overflow:"hidden"}}
              ,React.createElement('div',{style:{height:"100%",borderRadius:2,transition:"width 0.4f",width:`${pct}%`,background:s.fail>0?"#ef4444":pct===100&&s.total>0?"#22c55e":SWB_COLOR}})
            )
            ,React.createElement('div',{style:{fontSize:11,color:"#555",marginTop:4}},s.pass+s.na," / ",s.total," tested")
          )
          ,React.createElement('div',{style:{display:"flex",alignItems:"center",gap:8,marginLeft:16}}
            ,s.fail>0&&React.createElement('span',{style:SS.failBadge},s.fail," FAIL")
            ,s.untested>0&&s.total>0&&React.createElement('span',{style:SS.untestedBadge},s.untested," untested")
    ...
    ,showAdd
      ?React.createElement('div',{style:SS.addCard}
        ,React.createElement('div',{style:{display:"flex",gap:8,marginBottom:14}}
          ,React.createElement('button',{style:{...SS.tabBtn,...(tab==="manual"?{...SS.tabBtnActive,borderColor:SWB_COLOR,color:SWB_COLOR,background:"#1a1030"}:{})},onClick:()=>setTab("manual")},"✏️ Manual Entry")
          ,React.createElement('button',{style:{...SS.tabBtn,...(tab==="import"?{...SS.tabBtnActive,borderColor:SWB_COLOR,color:SWB_COLOR,background:"#1a1030"}:{})},onClick:()=>setTab("import")},"📥 Import Excel")
        )
        ,tab==="manual"&&React.createElement(React.Fragment,null
          ,React.createElement('div',{style:{fontSize:14,fontWeight:800,color:"#eee",marginBottom:12}},"New Site")
          ,[["SITE NAME","",newName,setNewName,"Site name"],["COMPANY (optional)","",newCo,setNewCo,"Company name"],["ABN (optional)","",newAbn,setNewAbn,"e.g. 12 345 678 901"],["ELECTRICAL LICENCE (optional)","",newLic,setNewLic,"e.g. 123456C"]].map(([lbl,,val,setter,ph])=>
          ...
          ,React.createElement('div',{style:{display:"flex",gap:8,marginTop:4}}
            ,React.createElement('button',{style:{...SS.ctaPrimary,background:SWB_COLOR},onClick:()=>{if(!newName.trim())return;onAddProject({id:swbSlug(newName),name:newName.trim(),company:newCo.trim(),abn:newAbn.trim(),licence:newLic.trim(),areas:[]},{});setNewName("");setNewCo("");setNewAbn("");setNewLic("");setShowAdd(false);}},"Add Site")
            ,React.createElement('button',{style:SS.ctaSecondary,onClick:()=>setShowAdd(false)},"Cancel")
          )
        )
        ,tab==="import"&&React.createElement(React.Fragment,null
          ,React.createElement('div',{style:{fontSize:14,fontWeight:800,color:"#eee",marginBottom:4}},"Import from Excel")
          ,React.createElement('div',{style:{fontSize:12,color:"#666",marginBottom:12}},"Columns needed: ",React.createElement('strong',{style:{color:"#aaa"}},"Area | Board / Panel Name")," — one row per board.")
          ,!importPreview&&React.createElement(React.Fragment,null
            ,React.createElement('input',{ref:fileRef,type:"file",accept:".xlsx,.xls,.csv",style:{display:"none"},onChange:handleFile})
            ,React.createElement('button',{style:{...SS.ctaPrimary,background:SWB_COLOR,width:"100%",marginBottom:8},onClick:()=>fileRef.current&&fileRef.current.click()},importing?"Parsing…":"📂 Choose Excel / CSV File")
          ...
          ,importPreview&&React.createElement(React.Fragment,null
            ,React.createElement('div',{style:{background:"#110a1a",border:`1px solid ${SWB_COLOR}44`,borderRadius:10,padding:"12px",marginBottom:12}}
              ,React.createElement('div',{style:{fontSize:12,fontWeight:700,color:SWB_COLOR,marginBottom:8}},"✓ Preview")
            ...
            ,[["SITE NAME","",importName,setImportName,"Site name"],["COMPANY (optional)","",importCo,setImportCo,"Company name"],["ABN (optional)","",importAbn,setImportAbn,"e.g. 12 345 678 901"],["ELECTRICAL LICENCE (optional)","",importLic,setImportLic,"e.g. 123456C"]].map(([lbl,,val,setter,ph])=>
              React.createElement('div',{key:lbl,style:{marginBottom:8}}
                ,React.createElement('div',{style:SS.metaLabelText},lbl)
                ,React.createElement('input',{style:{...SS.metaInput,marginTop:4},value:val,placeholder:ph,onChange:e=>setter(e.target.value)})
              )
            ...
            ,React.createElement('div',{style:{display:"flex",gap:8}}
              ,React.createElement('button',{style:{...SS.ctaPrimary,background:SWB_COLOR},onClick:confirmImport},"✓ Import Site")
      ...
      :React.createElement('button',{style:{...SS.ctaPrimary,background:SWB_COLOR,width:"100%",marginTop:8},onClick:()=>setShowAdd(true)},"+ Add / Import Site")
  );
}
...
// PROJECT HOME VIEW
// ─────────────────────────────────────────────────────────────────────────
function SWBProjectHomeView({project,meta,setMeta,results,onStartAudit,onReport,onManage,onHistory,onExport,onArchive,summary}){
  const[showExports,  setShowExports]  = React.useState(false);
  const[archiveMsg,   setArchiveMsg]   = React.useState("");
  const[confirmReset, setConfirmReset] = React.useState(false);
  const SS=swbStyles();
  ...
  const nextDue=swbAddYears(testDate,1);
  const totalBoards=(project.areas||[]).reduce((n,a)=>n+(a.boards||[]).length,0);
  const pct=summary.total>0?Math.round((summary.pass+summary.na)/summary.total*100):0;
  const handleArchive=()=>{onArchive();setArchiveMsg("Audit archived!");setTimeout(()=>setArchiveMsg(""),2500);};
  ...
  return React.createElement('div',{style:SS.homeWrap}
    ,React.createElement('div',{style:SS.brandBlock}
      ,React.createElement('div',{style:SS.brandTitle},"VORICK GROUP")
      ,React.createElement('div',{style:SS.brandSub},"Asset Maintenance")
    )
    ,React.createElement('div',{style:SS.siteTitle},project.name)
    ,React.createElement('div',{style:SS.siteSub},"Switchboard Audit — Annual")
    // Meta card
    ,React.createElement('div',{style:SS.metaCard}
      ...
      ,testDate&&React.createElement('div',{style:{...SS.duePill,borderColor:SWB_COLOR+"55",color:SWB_COLOR,display:"inline-block",marginTop:4,marginBottom:4}},"Next due: ",React.createElement('strong',null,nextDue))
    )
    // Stats
    ,React.createElement('div',{style:{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center",width:"100%",maxWidth:500}}
      ,[[`${totalBoards} boards`,"#aaa"],[`${summary.pass} pass`,"#22c55e"],[`${summary.fail} fail`,"#ef4444"],[`${summary.untested} untested`,"#f59e0b"]].map(([l,c])=>
        React.createElement('div',{key:l,style:{background:"#1a1a1a",border:`1px solid ${c}33`,borderRadius:8,padding:"8px 14px",fontSize:12,fontWeight:700,color:c}},l)
      )
    )
    // Progress bar
    ,React.createElement('div',{style:{width:"100%",maxWidth:500,height:4,background:"#2a2a2a",borderRadius:2,overflow:"hidden"}}
      ,React.createElement('div',{style:{height:"100%",width:pct+"%",background:SWB_COLOR,borderRadius:2,transition:"width 0.4s"}})
    )
    // Start button
    ,React.createElement('button',{
      style:{...SS.ctaPrimary,background:hasAuditor?SWB_COLOR:"#333",width:"100%",maxWidth:500,opacity:hasAuditor?1:0.5,cursor:hasAuditor?"pointer":"not-allowed"},
      onClick:()=>hasAuditor&&onStartAudit()
    },"☑ Start / Continue Audit")
    // Archive block
    ,React.createElement('div',{style:{width:"100%",maxWidth:500,background:"#161616",border:`1px solid ${SWB_COLOR}33`,borderRadius:12,padding:"10px 14px"}}
      ...
      ,React.createElement('button',{style:{...SS.smallBtn,width:"100%",color:SWB_COLOR,borderColor:SWB_COLOR+"44",padding:"8px"},onClick:handleArchive},"📁 Archive Switchboard Audit")
      ,archiveMsg&&React.createElement('div',{style:{fontSize:12,color:"#4ade80",marginTop:6,textAlign:"center"}},"✓ ",archiveMsg)
    )
    // Action buttons
    ,React.createElement('div',{style:{width:"100%",maxWidth:500,display:"flex",gap:8,flexWrap:"wrap"}}
      ...
      ,React.createElement('button',{style:{...SS.secondaryBtn,flex:1,color:"#a855f7",borderColor:"#a855f744"},onClick:onManage},"⚙ Manage")
      ,React.createElement('button',{style:{...SS.secondaryBtn,flex:1,color:"#4ade80",borderColor:"#22c55e44"},onClick:()=>setShowExports(x=>!x)},"↓ Export")
    )
    ,showExports&&React.createElement('button',{style:{...SS.exportBtn,width:"100%",maxWidth:500,color:SWB_COLOR,borderColor:SWB_COLOR+"44",background:"#110a1a"},onClick:onExport},"↓ Export Switchboard Audit (All Boards)")
  );
}

// ─────────────────────────────────────────────────────────────────────────
// AUDIT LIST VIEW — areas + boards in a flat list, no nesting
// ─────────────────────────────────────────────────────────────────────────
function SWBAuditListView({project,results,pid,onSelectBoard}){
  const SS=swbStyles();
  const areas=project.areas||[];
  if(areas.length===0)return React.createElement('div',{style:{...SS.listWrap,color:"#555",fontSize:14}},"No areas or boards yet. Go to ⚙ Manage to add them.");

  return React.createElement('div',{style:SS.listWrap}
    ,React.createElement('div',{style:SS.listTitle},"Select Board")
    ,areas.map(area=>React.createElement('div',{key:area.id,style:{marginBottom:8}}
      ,React.createElement('div',{style:{fontSize:10,color:"#666",fontWeight:700,letterSpacing:1,marginBottom:6,paddingLeft:2}},area.name.toUpperCase())
      ,(area.boards||[]).length===0&&React.createElement('div',{style:{color:"#555",fontSize:13,paddingLeft:4}},"No boards — add via ⚙ Manage")
      ,(area.boards||[]).map(board=>{
        const bs=swbBoardSummary(results,pid,area.id,board.id);
        const pct=bs.total>0?Math.round((bs.pass+bs.na)/bs.total*100):0;
        return React.createElement('button',{key:board.id,
          style:{...SS.siteCard,marginBottom:8,flexDirection:"column",gap:0,padding:0,overflow:"hidden",width:"100%",borderColor:bs.fail>0?"#ef444433":SWB_COLOR+"22"},
          onClick:()=>onSelectBoard(area.id,board.id)}
          ,React.createElement('div',{style:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 16px",width:"100%",boxSizing:"border-box"}}
            ,React.createElement('div',{style:{flex:1}}
              ,React.createElement('div',{style:SS.siteCardName},board.name)
              ,React.createElement('div',{style:SS.siteCardSub},bs.pass," pass · ",bs.fail," fail · ",bs.untested," untested")
              ,React.createElement('div',{style:{width:"100%",height:4,background:"#2a2a2a",borderRadius:2,marginTop:6,overflow:"hidden"}}
                ,React.createElement('div',{style:{height:"100%",borderRadius:2,width:`${pct}%`,background:bs.fail>0?"#ef4444":SWB_COLOR,transition:"width 0.4s"}})
              )
            )
            ,React.createElement('div',{style:{display:"flex",alignItems:"center",gap:8,marginLeft:16}}
              ,bs.fail>0&&React.createElement('span',{style:SS.failBadge},bs.fail)
              ,React.createElement('span',{style:SS.arrow},"›")
            )
          )
        );
...
// BOARD VIEW
// ─────────────────────────────────────────────────────────────────────────
function SWBBoardView({board,area,project,pid,results,activeItemKey,onCycle,onOpenItem,onCloseItem,onPatch}){
  const SS=swbStyles();
  const bs=swbBoardSummary(results,pid,area.id,board.id);
  ...
  return React.createElement('div',{style:{padding:"16px",position:"relative"}}
    ...
    ,React.createElement('div',{style:{fontSize:11,color:"#555",marginBottom:10}},"Tap status to cycle · tap note to add details")
    ,React.createElement('div',{style:{display:"flex",flexDirection:"column",gap:6}}
      ,SWB_CHECKLIST.map(({key,label})=>{
        const item=swbGetItem(results,pid,area.id,board.id,key);
        const st=item.status||"untested";
        const sm=SWB_SM[st];
        const hasDetail=item.defectId||item.comment||item.risk;
        return React.createElement('div',{key,style:{display:"flex",alignItems:"center",gap:8,background:"#161616",border:`1px solid ${sm.border}44`,borderRadius:10,padding:"10px 12px"}}
          ,React.createElement('button',{
            style:{width:64,flexShrink:0,padding:"6px 0",background:sm.bg,color:sm.fg,border:`1.5px solid ${sm.border}`,borderRadius:8,fontSize:11,fontWeight:800,cursor:"pointer"},
            onClick:()=>onCycle(key)
          },sm.label)
          ,React.createElement('div',{style:{flex:1,fontSize:14,fontWeight:600,color:"#eee"}},label)
          ,item.risk&&React.createElement('div',{style:{fontSize:11,fontWeight:800,color:SWB_RISK_COLORS[item.risk]||"#aaa",flexShrink:0}},item.risk)
          ,React.createElement('button',{
            style:{...SS.smallBtn,color:hasDetail?SWB_COLOR:"#555",borderColor:hasDetail?SWB_COLOR+"55":"#333",fontSize:11,padding:"5px 9px"},
            onClick:()=>onOpenItem(key)
          },hasDetail?"✎ edit":"+ note")
        );
      })
    )
    ,activeItemKey&&React.createElement(SWBItemModal,{
      itemKey:activeItemKey,board,area,pid,results,onPatch,onClose:onCloseItem,
    })
  );
}
...
// ITEM MODAL
// ─────────────────────────────────────────────────────────────────────────
function SWBItemModal({itemKey,board,area,pid,results,onPatch,onClose}){
  const item=swbGetItem(results,pid,area.id,board.id,itemKey);
  ...
  const save=()=>{onPatch(itemKey,{defectId,comment,risk});onClose();};

  return React.createElement('div',{style:SS.modalOverlay,onClick:e=>{if(e.target===e.currentTarget){save();onClose();}}}
    ,React.createElement('div',{style:SS.modalBox}
      ,React.createElement('div',{style:SS.modalHeader}
        ,React.createElement('div',null
          ,React.createElement('div',{style:SS.modalTitle},label)
          ,React.createElement('div',{style:SS.modalSub},board.name," · ",area.name)
        )
        ,React.createElement('div',{style:{padding:"5px 12px",background:sm.bg,color:sm.fg,border:`1.5px solid ${sm.border}`,borderRadius:8,fontSize:12,fontWeight:800}},sm.label)
      )
      ,React.createElement('div',{style:SS.modalField}
        ,React.createElement('label',{style:SS.modalLabel},"DEFECT ID")
        ,React.createElement('input',{style:SS.modalInput,value:defectId,placeholder:"e.g. 37",onChange:e=>setDefectId(e.target.value)})
      )
      ,React.createElement('div',{style:SS.modalField}
        ,React.createElement('label',{style:SS.modalLabel},"COMMENTS")
        ,React.createElement('textarea',{style:{...SS.modalInput,minHeight:80,resize:"vertical",fontFamily:"inherit"},value:comment,placeholder:"Observations, recommendations…",onChange:e=>setComment(e.target.value)})
      )
      ,React.createElement('div',{style:SS.modalField}
        ,React.createElement('label',{style:SS.modalLabel},"RISK RATING")
        ,React.createElement('div',{style:{display:"flex",gap:8,flexWrap:"wrap"}}
          ,[["","None"],["L","Low"],["M","Medium"],["H","High"],["U","Urgent"]].map(([r,label])=>
            React.createElement('button',{key:r||"none",
              style:{padding:"7px 14px",borderRadius:8,fontSize:13,fontWeight:700,cursor:"pointer",
                border:`1.5px solid ${r?(SWB_RISK_COLORS[r]||"#555")+"88":"#333"}`,
                background:risk===r?(r?SWB_RISK_COLORS[r]+"33":"#2a2a2a"):"transparent",
                color:risk===r?(r?SWB_RISK_COLORS[r]:"#eee"):"#666",
              },
              onClick:()=>setRisk(r)
            },label)
          )
      ...
      ,React.createElement('button',{style:{...SS.modalClose,background:SWB_COLOR,color:"#fff"},onClick:save},"Save")
    )
  );
}
...
// REPORT VIEW
// ─────────────────────────────────────────────────────────────────────────
function SWBReportView({project,results,pid,meta,onExport,onArchive}){
  const SS=swbStyles();
  const summary=swbSiteSummary(results,project);
  const testDate=meta.testDate||"";
  const nextDue=swbAddYears(testDate,1);
  const[archiveMsg,setArchiveMsg]=React.useState("");
  ...
  (project.areas||[]).forEach(area=>(area.boards||[]).forEach(board=>{
    SWB_CHECKLIST.forEach(({key,label})=>{
      const item=swbGetItem(results,pid,area.id,board.id,key);
      if((item.status||"untested")==="fail")fails.push({area:area.name,board:board.name,label,item});
    });
  }));

  return React.createElement('div',{style:SS.summaryWrap}
    ,React.createElement('div',{style:{...SS.summaryTitle,color:SWB_COLOR}},project.name)
    ,React.createElement('div',{style:SS.summaryMeta},"SWITCHBOARD AUDIT REPORT"+(meta.auditor?` · ${meta.auditor}`:""))
    ,testDate&&React.createElement('div',{style:{fontSize:12,color:SWB_COLOR,background:SWB_COLOR+"11",border:`1px solid ${SWB_COLOR}33`,borderRadius:8,padding:"8px 12px",marginBottom:16,marginTop:8}}
      ,"📅 Tested: ",swbFmtDate(testDate)," → next due: ",nextDue
    )
    ...
    ,React.createElement('div',{style:{marginBottom:20}}
      ,React.createElement('div',{style:{fontSize:12,fontWeight:700,color:SWB_COLOR,letterSpacing:0.8,marginBottom:8}},"BOARD SUMMARY")
      ,(project.areas||[]).map(area=>React.createElement('div',{key:area.id,style:{marginBottom:12}}
        ,React.createElement('div',{style:{fontSize:11,color:"#666",fontWeight:700,marginBottom:6,letterSpacing:0.5}},area.name.toUpperCase())
        ,(area.boards||[]).map(board=>{
          const bs=swbBoardSummary(results,pid,area.id,board.id);
          return React.createElement('div',{key:board.id,style:{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:"#161616",border:`1px solid ${bs.fail>0?"#ef444433":SWB_COLOR+"22"}`,borderRadius:8,marginBottom:4}}
    ...
    ,fails.length>0&&React.createElement('div',{style:{marginBottom:20}}
      ...
      ,fails.map((f,i)=>React.createElement('div',{key:i,style:{padding:"10px 12px",background:"#1e1010",border:"1px solid #ef444433",borderRadius:8,marginBottom:6,fontSize:13}}
        ,React.createElement('div',{style:{display:"flex",justifyContent:"space-between",marginBottom:4}}
          ...
          ,f.item.risk&&React.createElement('span',{style:{fontSize:11,fontWeight:800,color:SWB_RISK_COLORS[f.item.risk]||"#aaa",background:SWB_RISK_COLORS[f.item.risk]+"22",borderRadius:5,padding:"2px 6px"}},SWB_RISK_LABELS[f.item.risk]||f.item.risk)
        )
    ...
    ,React.createElement('button',{style:{...SS.exportBtn,width:"100%",marginBottom:8,color:SWB_COLOR,borderColor:SWB_COLOR+"44"},onClick:onExport},"📥 Export Excel (All Boards)")
    ,archiveMsg
      ?React.createElement('div',{style:{fontSize:12,color:"#22c55e",padding:"8px 0",textAlign:"center"}},archiveMsg)
      :React.createElement('button',{style:{...SS.smallBtn,width:"100%",color:"#555",padding:8},onClick:()=>{onArchive();setArchiveMsg("✓ Archived");setTimeout(()=>setArchiveMsg(""),3000);}},"Archive snapshot to history")
  );
}
...
function SWBManageView({project,onUpdateProject}){
  ...
  const[projName,      setProjName]      = React.useState(project.name);
  const[projCo,        setProjCo]        = React.useState(project.company||"");
  const[confirmDelArea,setConfirmDelArea]= React.useState(null);
  const[confirmDelBoard,setConfirmDelBoard]=React.useState(null);
  const SS=swbStyles();
  const upd=u=>onUpdateProject(u);
  ...
  const renameBoard=(aid,bid,name)=>upd({...project,areas:(project.areas||[]).map(a=>a.id===aid?{...a,boards:(a.boards||[]).map(b=>b.id===bid?{...b,name}:b)}:a)});
  ...
  return React.createElement('div',{style:SS.listWrap}
    ...
    ,React.createElement('div',{style:{background:"#161616",border:"1px solid #2a2a2a",borderRadius:12,padding:"12px 14px",marginBottom:14}}
      ,editingProject
        ?React.createElement(React.Fragment,null
          ,React.createElement('div',{style:{marginBottom:8}},React.createElement('div',{style:SS.metaLabelText},"SITE NAME"),React.createElement('input',{style:{...SS.metaInput,marginTop:4},value:projName,onChange:e=>setProjName(e.target.value)}))
          ,React.createElement('div',{style:{marginBottom:8}},React.createElement('div',{style:SS.metaLabelText},"COMPANY"),React.createElement('input',{style:{...SS.metaInput,marginTop:4},value:projCo,placeholder:"Company name",onChange:e=>setProjCo(e.target.value)}))
          ,React.createElement('div',{style:{display:"flex",gap:8}}
            ,React.createElement('button',{style:{...SS.ctaPrimary,background:SWB_COLOR},onClick:()=>{upd({...project,name:projName.trim()||project.name,company:projCo.trim()});setEditingProject(false);}},"Save")
            ,React.createElement('button',{style:SS.ctaSecondary,onClick:()=>setEditingProject(false)},"Cancel")
          )
        )
        :React.createElement('div',{style:{display:"flex",justifyContent:"space-between",alignItems:"center"}}
          ,React.createElement('div',{style:{fontSize:15,fontWeight:800,color:"#eee"}},project.name)
          ,React.createElement('button',{style:{...SS.smallBtn,color:"#a855f7",borderColor:"#a855f755"},onClick:()=>setEditingProject(true)},"✏️ Edit")
        )
    )
    // Areas section
    ,React.createElement('div',{style:{fontSize:11,color:"#666",letterSpacing:0.8,fontWeight:700,marginBottom:10}},"AREAS & BOARDS")
    ,(project.areas||[]).length===0&&React.createElement('div',{style:{color:"#555",fontSize:13,marginBottom:12}},"No areas yet.")
    ,(project.areas||[]).map(area=>React.createElement('div',{key:area.id,style:{border:`1px solid ${expandedArea===area.id?"#a855f755":"#2a2a2a"}`,borderRadius:12,marginBottom:10,overflow:"hidden"}}
      ,React.createElement('div',{style:{display:"flex",alignItems:"center",gap:8,padding:"12px 14px",background:"#1a1a1a"}}
        ,React.createElement('button',{style:{flex:1,display:"flex",alignItems:"center",gap:8,background:"transparent",border:"none",cursor:"pointer",color:"inherit",textAlign:"left",padding:0},onClick:()=>setExpandedArea(expandedArea===area.id?null:area.id)}
          ,React.createElement('span',{style:{fontSize:16,color:expandedArea===area.id?"#c084fc":"#aaa"}},expandedArea===area.id?"▾":"▸")
          ,React.createElement('span',{style:{fontWeight:700,color:"#eee",fontSize:14}},area.name)
          ,React.createElement('span',{style:{fontSize:11,color:"#555"}},(area.boards||[]).length," boards")
        )
        ,React.createElement('button',{style:{...SS.smallBtn,color:"#ef4444",borderColor:"#ef444433"},onClick:()=>setConfirmDelArea(area.id)},"🗑")
      )
      ,confirmDelArea===area.id&&React.createElement('div',{style:{background:"#2a1010",border:"1px solid #ef444466",padding:"10px 14px",display:"flex",flexDirection:"column",gap:8}}
        ,React.createElement('span',{style:{fontSize:13,color:"#f87171",fontWeight:700}},'Delete area "'+area.name+'"? This removes all boards.')
        ,React.createElement('div',{style:{display:"flex",gap:8}}
      ...
      ,expandedArea===area.id&&React.createElement('div',{style:{padding:"8px 12px 12px"}}
        ,(area.boards||[]).map(b=>React.createElement('div',{key:b.id,style:{display:"flex",alignItems:"center",gap:8,marginBottom:8}}
          ,confirmDelBoard===b.id
            ?React.createElement(React.Fragment,null
              ,React.createElement('span',{style:{fontSize:12,color:"#f87171",flex:1}},'Delete "'+b.name+'"?')
              ,React.createElement('button',{style:{...SS.smallBtn,color:"#f87171",borderColor:"#ef444466"},onClick:()=>{delBoard(area.id,b.id);setConfirmDelBoard(null);}},"Delete")
              ,React.createElement('button',{style:SS.smallBtn,onClick:()=>setConfirmDelBoard(null)},"Keep")
            )
            :React.createElement(React.Fragment,null
              ,React.createElement('input',{style:{...SS.smallInput,flex:1,fontSize:13},value:b.name,onChange:e=>renameBoard(area.id,b.id,e.target.value)})
              ,React.createElement('button',{style:{...SS.smallBtn,color:"#ef4444",borderColor:"#ef444433"},onClick:()=>setConfirmDelBoard(b.id)},"✕")
            )
        ))
        ,React.createElement('div',{style:{display:"flex",gap:8,marginTop:4}}
          ...
          ,React.createElement('button',{style:{...SS.smallBtn,color:SWB_COLOR,borderColor:SWB_COLOR+"55"},onClick:()=>addBoard(area.id)},"+ Add")
        )
      )
    ))
    ,React.createElement('div',{style:{display:"flex",gap:8,marginTop:4}}
      ...
      ,React.createElement('button',{style:{...SS.smallBtn,color:SWB_COLOR,borderColor:SWB_COLOR+"55"},onClick:addArea},"+ Add Area")
    )
  );
}
...
// HISTORY VIEW
// ─────────────────────────────────────────────────────────────────────────
function SWBHistoryView({history,onDelete}){
  const[expanded,setExpanded]=React.useState(null);
  const[deleteId,setDeleteId]=React.useState(null);
  const SS=swbStyles();
  if(history.length===0)return React.createElement('div',{style:{...SS.listWrap,color:"#555",fontSize:14}},"No archived audits yet. Archive from the Home screen after completing an audit.");
  return React.createElement('div',{style:SS.listWrap}
    ,React.createElement('div',{style:SS.listTitle},"History")
    ,history.map(snap=>React.createElement('div',{key:snap.id,style:{background:"#161616",border:"1px solid #2a2a2a",borderRadius:12,marginBottom:10,overflow:"hidden"}}
      ,React.createElement('div',{style:{padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",cursor:"pointer"},onClick:()=>setExpanded(expanded===snap.id?null:snap.id)}
        ,React.createElement('div',null
          ,React.createElement('div',{style:{fontSize:14,fontWeight:700,color:"#eee"}},swbFmtDate(snap.testDate)||"No date")
          ,React.createElement('div',{style:{fontSize:11,color:"#555",marginTop:2}},"Auditor: ",snap.auditor||"—"," · Archived: ",swbFmtDate(snap.archivedAt))
        )
        ,React.createElement('span',{style:{fontSize:18,color:"#555"}},expanded===snap.id?"▲":"▼")
      )
      ,expanded===snap.id&&React.createElement('div',{style:{padding:"0 16px 14px"}}
        ,deleteId===snap.id
          ?React.createElement('div',{style:{display:"flex",gap:8,alignItems:"center"}}
            ,React.createElement('span',{style:{fontSize:12,color:"#f87171",flex:1}},"Delete this snapshot?")
            ,React.createElement('button',{style:SS.confirmYes,onClick:()=>{onDelete(snap.id);setDeleteId(null);}},"Delete")
            ,React.createElement('button',{style:SS.confirmNo,onClick:()=>setDeleteId(null)},"Cancel")
          )
          :React.createElement('button',{style:{...SS.smallBtn,color:"#ef4444",borderColor:"#ef444433"},onClick:()=>setDeleteId(snap.id)},"Delete snapshot")
      )
    ))
  );
}
...
function SWBNavBtn({icon,label,active,onClick,color}){
  const c=active?(color||SWB_COLOR):"#555";
  return React.createElement('button',{onClick,style:{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3,background:"transparent",border:"none",cursor:"pointer",padding:"10px 0 6px",minHeight:50,color:c,borderTop:active?`2px solid ${color||SWB_COLOR}`:"2px solid transparent"}}
    ,React.createElement('span',{style:{fontSize:18}},icon)
    ,React.createElement('span',{style:{fontSize:9,fontWeight:active?700:500,letterSpacing:0.5}},label)
  );
}
...
function swbStyles(){
  return{
    ...
    appTitle:{fontSize:16,fontWeight:800,letterSpacing:1,color:SWB_COLOR},
    appSub:{fontSize:10,letterSpacing:0.5,transition:"color 0.3s"},
    backBtn:{fontSize:28,color:"#aaa",background:"transparent",border:"none",cursor:"pointer",lineHeight:1,padding:"0 8px 0 0",fontWeight:300},
    breadcrumb:{display:"flex",alignItems:"center",gap:6,padding:"8px 16px",background:"#161616",borderBottom:"1px solid #1e1e1e",fontSize:12,flexWrap:"wrap",flexShrink:0},
    bcItem:{color:"#888"},bcSep:{color:"#444"},
    main:{flex:1,overflowY:"auto",overflowX:"hidden",WebkitOverflowScrolling:"touch",minHeight:0},
    bottomNav:{display:"flex",background:"#161616",borderTop:"1px solid #222",flexShrink:0,paddingBottom:"34px",boxShadow:"0 200px 0 200px #161616"},
    homeWrap:{padding:"24px 16px",display:"flex",flexDirection:"column",alignItems:"center",gap:14},
    brandBlock:{textAlign:"center",borderBottom:`2px solid ${SWB_COLOR}`,paddingBottom:8,width:"100%",maxWidth:500},
    brandTitle:{fontSize:20,fontWeight:900,letterSpacing:3,color:SWB_COLOR},
    brandSub:{fontSize:11,color:"#666",letterSpacing:1,marginTop:2},
    siteTitle:{fontSize:20,fontWeight:800,color:"#eee"},
    siteSub:{fontSize:12,color:"#666"},
    metaCard:{width:"100%",maxWidth:500,background:"#161616",border:"1px solid #2a2a2a",borderRadius:14,padding:"14px"},
    metaLabelText:{fontSize:10,color:"#666",letterSpacing:0.8,fontWeight:700},
    metaInput:{background:"#1a1a1a",border:"1px solid #333",borderRadius:8,color:"#eee",padding:"9px 12px",fontSize:13,outline:"none",width:"100%",boxSizing:"border-box"},
    duePill:{fontSize:12,background:"#161616",border:"1px solid",borderRadius:8,padding:"5px 10px"},
    listWrap:{padding:"16px"},
    listTitle:{fontSize:20,fontWeight:800,color:"#eee",marginBottom:16},
    siteCard:{width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:14,padding:"16px 18px",marginBottom:10,cursor:"pointer",textAlign:"left"},
    siteCardName:{fontSize:16,fontWeight:700,color:"#eee"},
    siteCardSub:{fontSize:12,color:"#666",marginTop:2},
    failBadge:{fontSize:11,fontWeight:800,color:"#f87171",background:"#3d1a1a",borderRadius:6,padding:"3px 8px",border:"1px solid #ef4444"},
    untestedBadge:{fontSize:11,fontWeight:700,color:"#fbbf24",background:"#2a1e00",borderRadius:6,padding:"3px 8px"},
    arrow:{fontSize:22,color:"#555",lineHeight:1},
    addCard:{background:"#161616",border:"1px solid #2a2a2a",borderRadius:14,padding:"16px",marginBottom:10},
    tabBtn:{flex:1,padding:"9px",background:"#111",border:"1px solid #2a2a2a",borderRadius:8,color:"#666",fontSize:12,fontWeight:600,cursor:"pointer"},
    tabBtnActive:{background:"#1e1e1e",border:"1px solid",color:"#eee"},
    ctaPrimary:{padding:"11px 20px",background:SWB_COLOR,color:"#fff",border:"none",borderRadius:10,fontSize:14,fontWeight:800,cursor:"pointer"},
    ctaSecondary:{padding:"11px 20px",background:"#1a1a1a",color:"#aaa",border:"1px solid #333",borderRadius:10,fontSize:13,fontWeight:600,cursor:"pointer"},
    secondaryBtn:{padding:"11px",background:"#161616",color:"#aaa",border:"1px solid #2a2a2a",borderRadius:10,fontSize:13,fontWeight:700,cursor:"pointer"},
    exportBtn:{padding:"11px",background:"#161616",color:"#4ade80",border:"1px solid #2a2a2a",borderRadius:10,fontSize:13,fontWeight:700,cursor:"pointer"},
    smallBtn:{padding:"5px 10px",background:"transparent",border:"1px solid #333",borderRadius:6,fontSize:12,cursor:"pointer",fontWeight:600,flexShrink:0,color:"#aaa"},
    ...
    summaryMeta:{fontSize:13,color:"#777",marginTop:4},
    modalOverlay:{position:"absolute",inset:0,background:"rgba(0,0,0,0.88)",zIndex:200,display:"flex",alignItems:"flex-end",justifyContent:"center"},
    modalBox:{background:"#1a1a1a",border:"1px solid #333",borderRadius:"20px 20px 0 0",padding:"24px 20px 32px",width:"100%",maxWidth:620,maxHeight:"92vh",overflowY:"auto"},
    modalHeader:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20},
    modalTitle:{fontSize:18,fontWeight:800,color:"#eee"},
    modalSub:{fontSize:12,color:"#666",marginTop:3},
    modalField:{marginBottom:14},
    modalLabel:{display:"block",fontSize:10,color:"#666",letterSpacing:0.8,fontWeight:700,marginBottom:5},
    modalInput:{width:"100%",background:"#111",border:"1px solid #333",borderRadius:8,color:"#eee",padding:"10px 12px",fontSize:13,outline:"none",boxSizing:"border-box"},
    modalClose:{width:"100%",padding:"14px",border:"none",borderRadius:12,fontSize:15,fontWeight:800,cursor:"pointer",marginTop:8},
