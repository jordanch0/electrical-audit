...
// ─────────────────────────────────────────────────────────────────────────
// SWITCHBOARD / ENCLOSURE AUDIT MODULE v1
// ─────────────────────────────────────────────────────────────────────────

const SWB_COLOR = "#a855f7";
const K_SWB_PROJECTS = "swb-projects-v1";
const K_SWB_RESULTS  = "swb-results-v1";  // { siteId: { areaId: { boardId: { itemKey: {status,defectId,comment,risk} } } } }
...
// ─── Data helpers ─────────────────────────────────────────────────────────
function swbGetItem(results, siteId, areaId, boardId, itemKey) {
  return ((((results[siteId]||{})[areaId]||{})[boardId])||{})[itemKey]
    || { status:SWB_STATUS.UNTESTED, defectId:"", comment:"", risk:"" };
}
function swbGetStatus(results, siteId, areaId, boardId, itemKey) {
  return swbGetItem(results, siteId, areaId, boardId, itemKey).status || SWB_STATUS.UNTESTED;
}
function swbBoardSummary(results, siteId, areaId, boardId) {
  let pass=0,fail=0,na=0,untested=0;
  SWB_CHECKLIST.forEach(({key}) => {
    const v = swbGetStatus(results, siteId, areaId, boardId, key);
    if(v===SWB_STATUS.PASS)pass++;
    else if(v===SWB_STATUS.FAIL)fail++;
    else if(v===SWB_STATUS.NA)na++;
    else untested++;
  });
...
const SWB_C = {
  ...
  midGrey:   "FFD9D9D9",
  boardHdr:  "FF1E1E2E",
  // Priority colours matching RCD module exactly
  priorityU_bg:   "FF9B0000", priorityU_font: "FFFFFFFF",
  priorityH_bg:   "FFFFC7CE", priorityH_font: "FF9C0006",
  priorityM_bg:   "FFFFD966", priorityM_font: "FF7F6000",
  priorityL_bg:   "FFE2EFDA", priorityL_font: "FF375623",
};
function swbBorder(s="thin",c=SWB_C.midGrey){return{style:s,color:{rgb:c}};}
function swbAllBorders(){const b=swbBorder();return{top:b,bottom:b,left:b,right:b};}
function swbBtm(){return{bottom:swbBorder("hair")};}
function swbCS(fill,font={},align={},borders={}){
  return{fill:{patternType:"solid",fgColor:{rgb:fill}},font:{name:"Calibri",sz:10,...font},alignment:{vertical:"center",...align},border:borders};
}
function swbPriColor(p){
  if(p==="U")return{bg:SWB_C.priorityU_bg,font:SWB_C.priorityU_font,bold:true};
  if(p==="H")return{bg:SWB_C.priorityH_bg,font:SWB_C.priorityH_font,bold:false};
  if(p==="M")return{bg:SWB_C.priorityM_bg,font:SWB_C.priorityM_font,bold:false};
  if(p==="L")return{bg:SWB_C.priorityL_bg,font:SWB_C.priorityL_font,bold:false};
  return null;
}
function swbDS(ri, status, risk) {
  // Priority colour overrides if risk is set
  if(risk) {
    const pc = swbPriColor(risk);
    if(pc) return swbCS(pc.bg,{sz:10,color:{rgb:pc.font},bold:pc.bold},{wrapText:true},swbBtm());
  }
  // Alternate rows plain
  const bg = ri%2===0 ? SWB_C.white : SWB_C.lightGrey;
  return swbCS(bg,{sz:10,color:{rgb:SWB_C.darkGrey}},{wrapText:true},swbBtm());
}
function swbSetCell(ws,ref,value,style){
  const t=typeof value==="number"?"n":"s";
  ws[ref]={v:value!=null?value:"",t,s:style};
}

function exportSWBExcel(project, results, meta, logoBase64) {
  const wb = XLSX.utils.book_new();
  const ws = {};
  const range = { s:{r:0,c:0}, e:{r:0,c:5} };
  let r = 0;

  const sName = project.name || "Site";
  const testDate = (meta && meta.testDate) || "";
  const nextDue  = testDate ? (() => { try { const x=new Date(testDate); x.setFullYear(x.getFullYear()+1); return x.toLocaleDateString("en-AU",{day:"2-digit",month:"2-digit",year:"numeric"}); } catch(_){return "";} })() : "";
  ...
  const titleSt    = swbCS(SWB_C.darkGrey, {bold:true,sz:14,color:{rgb:SWB_C.white}}, {horizontal:"left"});
  const subtitleSt = swbCS("FF1E1E1E",     {sz:9, color:{rgb:"FFbbbbbb"}},             {horizontal:"left"});
  const metaSt     = swbCS("FF262626",     {sz:9, color:{rgb:"FF999999"}},             {horizontal:"left"});
  const spacerSt   = swbCS("FF1E1E1E");
  const hdrSt      = swbCS(SWB_C.purple,   {bold:true,sz:10,color:{rgb:SWB_C.white}}, {horizontal:"center",wrapText:true}, swbAllBorders());
  const boardHdrSt = swbCS(SWB_C.boardHdr, {bold:true,sz:11,color:{rgb:"FFc084fc"}},  {horizontal:"left"}, {bottom:swbBorder("medium","FFa855f7")});
  ...
  const n = 6; // columns A-F
  const cols = "ABCDEF".split("");

  // Row 1: Title
  swbSetCell(ws,"A1",`${sName}  –  Switchboard / Enclosure Audit`,titleSt);
  for(let c=1;c<n;c++) swbSetCell(ws,cols[c]+"1","",swbCS(SWB_C.darkGrey));
  ws["!merges"] = ws["!merges"]||[];
  ws["!merges"].push({s:{r:0,c:0},e:{r:0,c:n-1}});
  r=1;

  // Row 2: Company
  swbSetCell(ws,"A2",coLine,subtitleSt);
  for(let c=1;c<n;c++) swbSetCell(ws,cols[c]+"2","",swbCS("FF1E1E1E"));
  ws["!merges"].push({s:{r:1,c:0},e:{r:1,c:n-1}});
  r=2;

  // Row 3: Auditor / Date / Next Due
  swbSetCell(ws,"A3",`Auditor: ${(meta&&meta.auditor)||""}`,metaSt);
  swbSetCell(ws,"B3","",metaSt);
  swbSetCell(ws,"C3",`Date Tested: ${fmtD(testDate)}`,metaSt);
  swbSetCell(ws,"D3","",metaSt);
  swbSetCell(ws,"E3",`Next Annual Audit Due: ${nextDue}`,metaSt);
  swbSetCell(ws,"F3","",metaSt);
  ...
  // Row 4: Spacer
  for(let c=0;c<n;c++) swbSetCell(ws,cols[c]+"4","",spacerSt);
  ws["!merges"].push({s:{r:3,c:0},e:{r:3,c:n-1}});
  r=4;

  // ── Column headers ──
  const headers = ["Item","Pass / Fail","Defect ID","Comments","Risk","Area / Board"];
  headers.forEach((h,i)=>swbSetCell(ws,cols[i]+"5",h,hdrSt));
  r=5;
  ...
  (project.areas||[]).forEach(area => {
    (area.boards||[]).forEach(board => {
      ...
      swbSetCell(ws,cols[0]+(r+1),boardLabel,boardHdrSt);
      for(let c=1;c<n;c++) swbSetCell(ws,cols[c]+(r+1),"",boardHdrSt);
      ws["!merges"].push({s:{r,c:0},e:{r,c:n-1}});
      r++;

      // Checklist rows for this board
      SWB_CHECKLIST.forEach(({key,label}) => {
        const item = (((results[area.id]||{})[board.id])||{})[key] || {status:SWB_STATUS.UNTESTED,defectId:"",comment:"",risk:""};
        ...
        const rowStyle = swbDS(dataRowIdx, st, item.risk||"");

        swbSetCell(ws,cols[0]+(r+1),label,rowStyle);
        swbSetCell(ws,cols[1]+(r+1),pfLabel,swbCS((rowStyle.fill.fgColor.rgb),{sz:10,color:rowStyle.font.color,bold:rowStyle.font.bold},{horizontal:"center",wrapText:true},swbBtm()));
        swbSetCell(ws,cols[2]+(r+1),item.defectId||"",rowStyle);
        swbSetCell(ws,cols[3]+(r+1),item.comment||"",rowStyle);
        swbSetCell(ws,cols[4]+(r+1),item.risk||"",swbCS((rowStyle.fill.fgColor.rgb),{sz:10,color:rowStyle.font.color,bold:rowStyle.font.bold},{horizontal:"center"},swbBtm()));
        swbSetCell(ws,cols[5]+(r+1),boardLabel,rowStyle);

        r++;
        dataRowIdx++;
      });

      // Blank spacer row between boards (for photo insertion)
      const spacer2 = swbCS("FF181820");
      swbSetCell(ws,cols[0]+(r+1),"← Insert photos here",swbCS("FF181820",{sz:9,color:{rgb:"FF444466"},italic:true},{horizontal:"left"}));
      for(let c=1;c<n;c++) swbSetCell(ws,cols[c]+(r+1),"",spacer2);
      ws["!merges"].push({s:{r,c:0},e:{r,c:n-1}});
      r++;
    });
  });
  ...
  // Taller rows for checklist items (row 5 onward)
  if(ws["!rows"][4]) ws["!rows"][4].hpt = 22; // header row
...
function SWBApp({ onGoHome }) {
  ...
  const [saveFlash,   setSaveFlash]   = React.useState(false);
  const [activeProject, setActiveProject] = React.useState(null);
  const [view,        setView]        = React.useState("projects"); // projects | home | area | board | report | manage | history
  const [activeAreaId,  setActiveAreaId]  = React.useState(null);
  const [activeBoardId, setActiveBoardId] = React.useState(null);
  const [activeItemKey, setActiveItemKey] = React.useState(null); // item open in modal

  React.useEffect(()=>{
    (async()=>{
      try {
        const [p,r,m,h] = await Promise.all([
          load(K_SWB_PROJECTS,[]),
          load(K_SWB_RESULTS,{}),
          load(K_SWB_META,{}),
          load(K_SWB_HISTORY,[]),
  ...
  const patchItem = (areaId, boardId, itemKey, patch) => {
    setAllResults(prev=>{
      const site = prev[activeProject]||{};
      const ar   = site[areaId]||{};
      const bd   = ar[boardId]||{};
      const old  = bd[itemKey]||{status:SWB_STATUS.UNTESTED,defectId:"",comment:"",risk:""};
      return {...prev,[activeProject]:{...site,[areaId]:{...ar,[boardId]:{...bd,[itemKey]:{...old,...patch}}}}};
    });
  };

  const cycleItem = (areaId, boardId, itemKey) => {
    const cur = swbGetStatus(allResults, activeProject, areaId, boardId, itemKey);
    const next = cur===SWB_STATUS.UNTESTED?SWB_STATUS.PASS:cur===SWB_STATUS.PASS?SWB_STATUS.FAIL:cur===SWB_STATUS.FAIL?SWB_STATUS.NA:SWB_STATUS.UNTESTED;
    patchItem(areaId, boardId, itemKey, {status:next});
  };
  ...
  const goArea     = ()=>{ setView("area"); setActiveBoardId(null); setActiveItemKey(null); };
  ...
  return React.createElement('div',{style:SS.root}
    ...
    ,React.createElement('header',{style:{...SS.topbar, borderBottom:`2px solid ${SWB_COLOR}33`}}
      ,React.createElement('div',{style:SS.topbarLeft}
        ,view!=="projects"&&React.createElement('button',{style:SS.backBtn, onClick:()=>{
          if(view==="board"){goArea();}
          else if(view==="area"){goHome();}
      ...
      ,React.createElement('div',{style:SS.topbarRight}
        ...
        ,React.createElement('button',{style:{...SS.smallBtn,color:SWB_COLOR,borderColor:SWB_COLOR+"44",fontSize:11},onClick:onGoHome},"⌂ Modules")
      )
    )

    // ── Breadcrumb
    ,view!=="projects"&&React.createElement('div',{style:SS.breadcrumb}
      ,React.createElement('span',{style:{...SS.bcItem,color:SWB_COLOR},onClick:goHome},project?.name||"")
      ,view==="area"&&area&&React.createElement(React.Fragment,null
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:SWB_COLOR}},area.name)
      )
      ,view==="board"&&board&&React.createElement(React.Fragment,null
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:"#aaa"},onClick:goArea},area?.name||"")
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:SWB_COLOR}},board.name)
      )
      ,view==="manage"&&React.createElement(React.Fragment,null
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:"#a855f7"}},"Manage")
      )
      ,view==="history"&&React.createElement(React.Fragment,null
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:"#f59e0b"}},"History")
      )
      ,view==="report"&&React.createElement(React.Fragment,null
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:SWB_COLOR}},"Report")
      )
    )
    ...
    ,React.createElement('div',{style:SS.main}
      ...
      ,view==="area"&&project&&React.createElement(SWBAreaListView,{
        project, results:allResults, projectId:activeProject,
        onSelect: aid=>{ setActiveAreaId(aid); setView("board"); setActiveBoardId(null); },
      ...
      ,view==="board"&&board&&React.createElement(SWBBoardView,{
        board, area, project,
        results:allResults, projectId:activeProject,
        activeItemKey,
      ...
      ,view==="report"&&project&&React.createElement(SWBReportView,{
        project, results:allResults, projectId:activeProject, meta,
        onExport: ()=>exportSWBExcel(project, allResults[activeProject]||{}, meta),
        onArchive: archiveAudit,
      })
    ...
    ,view!=="projects"&&React.createElement('nav',{style:SS.bottomNav}
      ...
      ,React.createElement(SWBNavBtn,{icon:"☑",label:"Audit",active:view==="area"||view==="board",onClick:()=>setView("area"),color:SWB_COLOR})
...
function SWBProjectListView({projects, allResults, onSelect, onAddProject, onDeleteProject}) {
  ...
  return React.createElement('div',{style:SS.listWrap}
    ,React.createElement('div',{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}
      ,React.createElement('div',{style:SS.listTitle},"Switchboard Audit")
      ,React.createElement('button',{style:{...SS.smallBtn,color:SWB_COLOR,borderColor:SWB_COLOR+"44"},onClick:()=>setShowAdd(!showAdd)},"+ New Site")
    )

    ,showAdd&&React.createElement('div',{style:SS.addCard}
      ,React.createElement('div',{style:{fontSize:13,fontWeight:700,color:"#eee",marginBottom:10}},"New Site")
      ,[
        ["Site Name","text",newName,setNewName,"e.g. Dixon Park"],
        ["Company","text",newCo,setNewCo,""],
        ["ABN","text",newAbn,setNewAbn,""],
        ["Electrical Licence","text",newLic,setNewLic,""],
      ].map(([lbl,type,val,setVal,ph])=>
        React.createElement('div',{key:lbl,style:{marginBottom:8}}
          ,React.createElement('div',{style:SS.metaLabelText},lbl)
          ,React.createElement('input',{style:{...SS.metaInput,marginTop:4},type,value:val,placeholder:ph,onChange:e=>setVal(e.target.value)})
        )
      )
      ,React.createElement('button',{
        style:{...SS.ctaPrimary,background:SWB_COLOR,width:"100%",marginTop:4},
        onClick:()=>{
          if(!newName.trim())return;
          onAddProject({id:swbSlug(newName),name:newName.trim(),company:newCo.trim(),abn:newAbn.trim(),licence:newLic.trim(),areas:[]});
          setNewName(""); setShowAdd(false);
        }
      },"Create Site")
    )

    ,projects.length===0&&!showAdd&&React.createElement('div',{style:{color:"#555",fontSize:14,marginTop:20}},"No sites yet. Tap + New Site to begin.")

    ,projects.map(p=>{
      const s = swbSiteSummary(allResults, p);
      const totalBoards = (p.areas||[]).reduce((n,a)=>n+(a.boards||[]).length,0);
      const pct = s.total>0?Math.round((s.pass+s.na)/s.total*100):0;
      const hasFail = s.fail>0;
      return React.createElement('div',{key:p.id}
        ,React.createElement('button',{
          style:{...SS.siteCard,...(hasFail?SS.siteCardFail:{})},
          onClick:()=>{ if(deleteId===p.id)return; onSelect(p.id); }
        }
          ,React.createElement('div',{style:SS.siteCardLeft}
            ,React.createElement('div',{style:SS.siteCardName},p.name)
            ,React.createElement('div',{style:SS.siteCardSub},p.company||"",` · ${(p.areas||[]).length} areas · ${totalBoards} boards`)
            ,React.createElement('div',{style:SS.siteCardBar}
              ,React.createElement('div',{style:{...SS.siteCardBarFill,width:pct+"%",background:hasFail?"#ef4444":SWB_COLOR}})
            )
          )
          ,React.createElement('div',{style:SS.siteCardRight}
            ,s.fail>0&&React.createElement('div',{style:SS.failBadge},s.fail+" FAIL")
            ,s.untested>0&&React.createElement('div',{style:SS.untestedBadge},s.untested+" untested")
            ,React.createElement('span',{style:SS.arrow},"›")
          )
        )
        ,deleteId===p.id
          ?React.createElement('div',{style:{display:"flex",gap:8,marginTop:-6,marginBottom:8,padding:"8px 4px"}}
            ,React.createElement('span',{style:{fontSize:12,color:"#f87171",flex:1,alignSelf:"center"}},"Delete this site?")
            ,React.createElement('button',{style:SS.confirmYes,onClick:()=>{ onDeleteProject(p.id); setDeleteId(null); }},"Delete")
            ,React.createElement('button',{style:SS.confirmNo,onClick:()=>setDeleteId(null)},"Cancel")
          )
          :React.createElement('button',{style:{...SS.resetBtn,display:"block",textAlign:"right",width:"100%",marginTop:-4,marginBottom:6},onClick:e=>{e.stopPropagation();setDeleteId(p.id);}},"delete")
      );
    })
  );
}
...
function SWBProjectHomeView({project, meta, setMeta, results, onStartAudit, onReport, onManage, onHistory, onExport, onArchive, summary}) {
  ...
  return React.createElement('div',{style:SS.homeWrap}
    ,React.createElement('div',{style:{...SS.brandBlock,borderColor:SWB_COLOR}}
      ,React.createElement('div',{style:{...SS.brandTitle,color:SWB_COLOR}},"SWITCHBOARD AUDIT")
      ,React.createElement('div',{style:SS.brandSub},"VORICK GROUP ASSET MAINTENANCE")
    )
    ,React.createElement('div',{style:{textAlign:"center"}}
      ,React.createElement('div',{style:SS.siteTitle},project.name)
      ,React.createElement('div',{style:SS.siteSub},project.company||"",(project.abn?` · ABN: ${project.abn}`:""))
    )
    ...
    // Progress
    ,React.createElement('div',{style:{width:"100%",maxWidth:500,height:6,background:"#222",borderRadius:3,overflow:"hidden"}}
      ,React.createElement('div',{style:{height:"100%",width:pct+"%",background:SWB_COLOR,borderRadius:3,transition:"width 0.4s"}})
    )
    ,React.createElement('div',{style:{fontSize:12,color:"#555"}},pct+"% complete")

    // Meta card
    ,React.createElement('div',{style:{...SS.metaCard,width:"100%",maxWidth:500}}
      ,React.createElement('div',{style:{fontSize:12,fontWeight:700,color:SWB_COLOR,marginBottom:10}},"AUDIT DETAILS")
      ,[
        ["Auditor",     "text",   meta.auditor||"",    v=>setMeta({auditor:v})],
        ["Test Date",   "date",   meta.testDate||"",   v=>setMeta({testDate:v})],
        ["Notes",       "text",   meta.notes||"",      v=>setMeta({notes:v})],
      ].map(([lbl,type,val,setV])=>
        React.createElement('div',{key:lbl,style:{marginBottom:8}}
          ,React.createElement('div',{style:SS.metaLabelText},lbl)
          ,React.createElement('input',{style:{...SS.metaInput,marginTop:4},type,value:val,onChange:e=>setV(e.target.value)})
        )
      )
      ,testDate&&React.createElement('div',{style:{fontSize:11,color:"#555",marginTop:4}},"Next audit due: ",React.createElement('span',{style:{color:SWB_COLOR,fontWeight:700}},nextDue))
    )

    // Action buttons
    ,React.createElement('button',{style:{...SS.ctaPrimary,background:SWB_COLOR,width:"100%",maxWidth:500},onClick:onStartAudit},"☑ Start / Continue Audit")
    ,React.createElement('div',{style:{display:"flex",gap:10,width:"100%",maxWidth:500}}
      ,React.createElement('button',{style:{...SS.secondaryBtn,flex:1},onClick:onReport},"≡ Report")
      ,React.createElement('button',{style:{...SS.secondaryBtn,flex:1},onClick:onManage},"⚙ Manage")
      ,React.createElement('button',{style:{...SS.secondaryBtn,flex:1,color:"#f59e0b",borderColor:"#f59e0b44"},onClick:onHistory},"🕐 History")
    )
    ,React.createElement('button',{style:{...SS.exportBtn,width:"100%",maxWidth:500,color:SWB_COLOR,borderColor:SWB_COLOR+"44"},onClick:onExport},"📥 Export Excel")

    ,archiveMsg?React.createElement('div',{style:{fontSize:12,color:"#22c55e",padding:"8px 0"}},archiveMsg)
    :React.createElement('button',{style:{...SS.resetBtn,color:"#666"},onClick:()=>{
      onArchive();
...
// ─────────────────────────────────────────────────────────────────────────
// AREA LIST VIEW  (shows areas; tapping drills into boards)
// ─────────────────────────────────────────────────────────────────────────
function SWBAreaListView({project, results, projectId, onSelectBoard}) {
  const SS = swbStyles(SWB_COLOR);
  return React.createElement('div',{style:SS.listWrap}
    ,React.createElement('div',{style:SS.listTitle},"Select Area")
    ,(project.areas||[]).length===0&&React.createElement('div',{style:{color:"#555",fontSize:14}},"No areas yet. Go to ⚙ Manage to add areas and boards.")
    ,(project.areas||[]).map(area=>{
      const boards = area.boards||[];
      const totalItems = boards.length * SWB_CHECKLIST.length;
      let pass=0,fail=0,untested=0;
      boards.forEach(b=>{
        const s=swbBoardSummary(results,project.id,area.id,b.id);
        pass+=s.pass; fail+=s.fail; untested+=s.untested;
      });
      const pct = totalItems>0?Math.round((pass)/totalItems*100):0;
      return React.createElement('div',{key:area.id}
        ,React.createElement('div',{style:{...SS.siteCard,cursor:"default",marginBottom:4,flexDirection:"column",alignItems:"flex-start",gap:8}}
          ,React.createElement('div',{style:{fontWeight:700,color:"#eee",fontSize:15}},area.name)
          ,React.createElement('div',{style:{fontSize:12,color:"#555"}},boards.length," boards")
          ,fail>0&&React.createElement('div',{style:SS.failBadge},fail+" FAIL")
        )
        ,boards.map(b=>{
          const bs=swbBoardSummary(results,project.id,area.id,b.id);
          const bpct=bs.total>0?Math.round((bs.pass+bs.na)/bs.total*100):0;
          return React.createElement('button',{
            key:b.id,
            style:{...SS.siteCard,marginLeft:16,marginBottom:8,width:"calc(100% - 16px)",borderColor:bs.fail>0?"#ef444433":SWB_COLOR+"22"},
            onClick:()=>onSelectBoard(area.id,b.id)
          }
            ,React.createElement('div',{style:SS.siteCardLeft}
              ...
              ,React.createElement('div',{style:SS.siteCardBar}
                ,React.createElement('div',{style:{...SS.siteCardBarFill,width:bpct+"%",background:bs.fail>0?"#ef4444":SWB_COLOR}})
              )
            )
            ,React.createElement('div',{style:SS.siteCardRight}
              ,bs.fail>0&&React.createElement('div',{style:SS.failBadge},bs.fail)
              ,React.createElement('span',{style:SS.arrow},"›")
            )
          );
        })
...
// ─────────────────────────────────────────────────────────────────────────
// BOARD VIEW  (checklist for one board + item detail modal)
// ─────────────────────────────────────────────────────────────────────────
function SWBBoardView({board, area, project, results, projectId, activeItemKey, onCycle, onOpenItem, onCloseItem, onPatch}) {
  const SS = swbStyles(SWB_COLOR);
  const bs = swbBoardSummary(results, projectId, area.id, board.id);
  ...
  return React.createElement('div',{style:{padding:"16px",position:"relative"}}
    ...
    ,React.createElement('div',{style:{display:"flex",flexDirection:"column",gap:6}}
      ,SWB_CHECKLIST.map(({key,label})=>{
        const item = swbGetItem(results, projectId, area.id, board.id, key);
        const st   = item.status || SWB_STATUS.UNTESTED;
        const sm   = SWB_SM[st];
        ...
        return React.createElement('div',{key,style:{display:"flex",alignItems:"center",gap:8,background:"#161616",border:`1px solid ${sm.border}44`,borderRadius:10,padding:"10px 12px"}}
          // Status toggle
          ,React.createElement('button',{
            style:{width:70,flexShrink:0,padding:"6px 0",background:sm.bg,color:sm.fg,border:`1.5px solid ${sm.border}`,borderRadius:8,fontSize:12,fontWeight:800,cursor:"pointer"},
            onClick:()=>onCycle(key)
          },sm.label)
          // Label
          ,React.createElement('div',{style:{flex:1,fontSize:14,fontWeight:600,color:"#eee"}},label)
          // Risk badge (if set)
          ,item.risk&&React.createElement('div',{style:{fontSize:11,fontWeight:800,color:SWB_RISK_COLORS[item.risk]||"#aaa",background:SWB_COLOR+"11",borderRadius:5,padding:"2px 6px",flexShrink:0}},item.risk)
          // Edit note button
          ,React.createElement('button',{
    ...
    ,activeItemKey&&React.createElement(SWBItemModal,{
      ...
      board, area, project, results, projectId,
      onPatch, onClose:onCloseItem,
    })
  );
}
...
// ITEM DETAIL MODAL
// ─────────────────────────────────────────────────────────────────────────
function SWBItemModal({itemKey, board, area, project, results, projectId, onPatch, onClose}) {
  const item  = swbGetItem(results, projectId, area.id, board.id, itemKey);
    ...
    ,React.createElement('div',{style:SS.modalBox}
      ...
      ,React.createElement('div',{style:SS.modalField}
        ...
        ,React.createElement('textarea',{
          style:{...SS.modalInput,minHeight:80,resize:"vertical",fontFamily:"inherit"},
          value:comment,
...
// REPORT VIEW
// ─────────────────────────────────────────────────────────────────────────
function SWBReportView({project, results, projectId, meta, onExport, onArchive}) {
  const SS = swbStyles(SWB_COLOR);
  ...
  (project.areas||[]).forEach(area=>(area.boards||[]).forEach(board=>{
    SWB_CHECKLIST.forEach(({key,label})=>{
      ...
      if((item.status||SWB_STATUS.UNTESTED)===SWB_STATUS.FAIL){
        fails.push({area:area.name, board:board.name, label, item});
      }
    });
  }));
  ...
  return React.createElement('div',{style:SS.summaryWrap}
    ...
    ,testDate&&React.createElement('div',{style:{fontSize:12,color:SWB_COLOR,background:SWB_COLOR+"11",border:`1px solid ${SWB_COLOR}33`,borderRadius:8,padding:"8px 12px",marginBottom:16}}
      ,"📅 Tested: ",fmtD(testDate)," → next due: ",nextDue
    )
    ...
    ,React.createElement('div',{style:{marginBottom:20}}
      ...
      ,(project.areas||[]).map(area=>
        React.createElement('div',{key:area.id,style:{marginBottom:12}}
...
function SWBManageView({project, onUpdateProject}) {
  const [newAreaName,   setNewAreaName]   = React.useState("");
  const [newBoardNames, setNewBoardNames] = React.useState({});
  ...
  const addArea = () => {
    if(!newAreaName.trim()) return;
    upd({...project, areas:[...(project.areas||[]),{id:swbSlug(newAreaName),name:newAreaName.trim(),boards:[]}]});
    setNewAreaName("");
  };
  const deleteArea = aid => upd({...project, areas:(project.areas||[]).filter(a=>a.id!==aid)});
  const addBoard = aid => {
    const n = (newBoardNames[aid]||"").trim();
    if(!n) return;
    upd({...project, areas:(project.areas||[]).map(a=>a.id===aid?{...a,boards:[...(a.boards||[]),{id:swbSlug(n),name:n}]}:a)});
    setNewBoardNames(x=>({...x,[aid]:""}));
  };
  const deleteBoard = (aid,bid) => upd({...project, areas:(project.areas||[]).map(a=>a.id===aid?{...a,boards:(a.boards||[]).filter(b=>b.id!==bid)}:a)});
  ...
  return React.createElement('div',{style:SS.listWrap}
    ,React.createElement('div',{style:SS.listTitle},"Manage")
    // Project details
    ,React.createElement('div',{style:{...SS.addCard,marginBottom:16}}
      ,React.createElement('div',{style:{fontSize:12,fontWeight:700,color:SWB_COLOR,marginBottom:8}},"SITE DETAILS")
      ,[["Site Name","name"],["Company","company"],["ABN","abn"],["Electrical Licence","licence"]].map(([lbl,field])=>
        React.createElement('div',{key:field,style:{marginBottom:8}}
          ,React.createElement('div',{style:SS.metaLabelText},lbl)
          ,React.createElement('input',{style:{...SS.metaInput,marginTop:4},value:project[field]||"",onChange:e=>upd({...project,[field]:e.target.value})})
        )
      )
    )
    // Areas
    ,React.createElement('div',{style:{fontSize:12,fontWeight:700,color:SWB_COLOR,marginBottom:8,letterSpacing:0.8}},"AREAS & BOARDS")
    ,(project.areas||[]).map(area=>
      React.createElement('div',{key:area.id,style:{...SS.manageSection,borderColor:SWB_COLOR+"33",marginBottom:10}}
        ,React.createElement('div',{style:{...SS.manageSectionHeader,background:"#1a1a1a",justifyContent:"space-between"}}
          ,React.createElement('div',{style:{fontWeight:700,color:"#eee"}},area.name)
          ,React.createElement('div',{style:{display:"flex",gap:8}}
            ,React.createElement('button',{style:{...SS.smallBtn,fontSize:11},onClick:()=>setExpanded(x=>({...x,[area.id]:!x[area.id]}))},expanded[area.id]?"▲ collapse":"▼ boards")
            ,React.createElement('button',{style:{...SS.smallBtn,color:"#ef4444",borderColor:"#ef444433",fontSize:11},onClick:()=>deleteArea(area.id)},"✕ area")
          )
        )
        ,expanded[area.id]&&React.createElement('div',{style:{padding:"10px 12px"}}
          ,(area.boards||[]).map(b=>
            React.createElement('div',{key:b.id,style:{display:"flex",alignItems:"center",gap:8,marginBottom:8}}
              ...
              ,React.createElement('button',{style:{...SS.smallBtn,color:"#ef4444",borderColor:"#ef444433"},onClick:()=>deleteBoard(area.id,b.id)},"✕")
            )
          )
          ,React.createElement('div',{style:{display:"flex",gap:8,marginTop:4}}
            ,React.createElement('input',{style:{...SS.smallInput,flex:1},placeholder:"New board name (e.g. MCC 1)",value:newBoardNames[area.id]||"",onChange:e=>setNewBoardNames(x=>({...x,[area.id]:e.target.value})),onKeyDown:e=>{if(e.key==="Enter")addBoard(area.id);}})
...
// HISTORY VIEW
// ─────────────────────────────────────────────────────────────────────────
function SWBHistoryView({history, project, onDelete}) {
  const [expanded, setExpanded] = React.useState(null);
  const [deleteId, setDeleteId] = React.useState(null);
  const SS = swbStyles(SWB_COLOR);
  const fmtD = d => { if(!d) return ""; try { return new Date(d).toLocaleDateString("en-AU",{day:"2-digit",month:"2-digit",year:"numeric"}); } catch(_){return d;} };

  if(history.length===0) return React.createElement('div',{style:{...SS.listWrap,color:"#555",fontSize:14}},"No archived audits yet. Tap \"Archive\" on the project home screen to save a snapshot.");

  return React.createElement('div',{style:SS.listWrap}
    ,React.createElement('div',{style:SS.listTitle},"History")
    ,history.map(snap=>
      React.createElement('div',{key:snap.id,style:{background:"#161616",border:"1px solid #2a2a2a",borderRadius:12,marginBottom:10,overflow:"hidden"}}
        ,React.createElement('div',{style:{padding:"14px 16px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",cursor:"pointer"},onClick:()=>setExpanded(expanded===snap.id?null:snap.id)}
          ,React.createElement('div',null
            ,React.createElement('div',{style:{fontSize:14,fontWeight:700,color:"#eee"}},fmtD(snap.testDate)||"No date")
            ,React.createElement('div',{style:{fontSize:11,color:"#555",marginTop:2}},"Auditor: ",snap.auditor||"—"," · Archived: ",fmtD(snap.archivedAt))
          )
        ...
        ,expanded===snap.id&&React.createElement('div',{style:{padding:"0 16px 14px"}}
          ...
          ,React.createElement('div',{style:{fontSize:12,color:"#666",marginBottom:10}},"Snapshot — read only")
          ,deleteId===snap.id
...
// STYLES FACTORY
// ─────────────────────────────────────────────────────────────────────────
function swbStyles(color) {
  return {
    root:{display:"flex",flexDirection:"column",flex:1,minHeight:0,touchAction:"pan-y",background:"#111",color:"#eee",fontFamily:"'DM Sans','SF Pro Display',-apple-system,sans-serif",WebkitFontSmoothing:"antialiased",overflow:"hidden"},
    topbar:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px 10px",background:"#161616",flexShrink:0,zIndex:10},
    topbarLeft:{display:"flex",alignItems:"center",gap:12},
    topbarRight:{display:"flex",alignItems:"center",gap:8},
    appTitle:{fontSize:16,fontWeight:800,letterSpacing:1,color},
    ...
    bottomNav:{display:"flex",background:"#161616",borderTop:"1px solid #222",flexShrink:0,paddingBottom:"34px",boxShadow:"0 200px 0 200px #161616"},
    homeWrap:{padding:"24px 16px",display:"flex",flexDirection:"column",alignItems:"center",gap:14},
    brandBlock:{textAlign:"center",borderBottom:`2px solid ${color}`,paddingBottom:8,width:"100%",maxWidth:500},
    brandTitle:{fontSize:20,fontWeight:900,letterSpacing:3,color},brandSub:{fontSize:11,color:"#666",letterSpacing:1,marginTop:2},
    siteTitle:{fontSize:20,fontWeight:800,color:"#eee"},siteSub:{fontSize:12,color:"#666"},
    metaCard:{background:"#161616",border:"1px solid #2a2a2a",borderRadius:14,padding:"14px"},
    metaLabelText:{fontSize:10,color:"#666",letterSpacing:0.8,fontWeight:700},
    metaInput:{background:"#1a1a1a",border:"1px solid #333",borderRadius:8,color:"#eee",padding:"9px 12px",fontSize:13,outline:"none",width:"100%",boxSizing:"border-box"},
    listWrap:{padding:"16px"},listTitle:{fontSize:20,fontWeight:800,color:"#eee",marginBottom:16},
    siteCard:{width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",background:"#1a1a1a",border:"1px solid #2a2a2a",borderRadius:14,padding:"16px 18px",marginBottom:10,cursor:"pointer",textAlign:"left"},
    siteCardFail:{background:"#1e1010",borderColor:"#ef444455"},
    siteCardLeft:{flex:1},siteCardRight:{display:"flex",alignItems:"center",gap:8,marginLeft:16},
    siteCardName:{fontSize:16,fontWeight:700,color:"#eee"},siteCardSub:{fontSize:12,color:"#666",marginTop:2},
    siteCardBar:{width:"100%",height:4,background:"#2a2a2a",borderRadius:2,marginTop:10,overflow:"hidden"},
    siteCardBarFill:{height:"100%",borderRadius:2,transition:"width 0.4s"},
    ...
    modalInput:{width:"100%",background:"#111",border:"1px solid #333",borderRadius:8,color:"#eee",padding:"10px 12px",fontSize:13,outline:"none",boxSizing:"border-box"},
    modalClose:{width:"100%",padding:"14px",border:"none",borderRadius:12,fontSize:15,fontWeight:800,cursor:"pointer",marginTop:8},
    ctaPrimary:{padding:"11px 20px",background:color,color:"#fff",border:"none",borderRadius:10,fontSize:14,fontWeight:800,cursor:"pointer"},
    ctaSecondary:{padding:"11px 20px",background:"#1a1a1a",color:"#aaa",border:"1px solid #333",borderRadius:10,fontSize:13,fontWeight:600,cursor:"pointer"},
    secondaryBtn:{padding:"11px",background:"#161616",color:"#aaa",border:"1px solid #2a2a2a",borderRadius:10,fontSize:13,fontWeight:700,cursor:"pointer"},
    exportBtn:{padding:"11px",background:"#161616",color:"#4ade80",border:"1px solid #2a2a2a",borderRadius:10,fontSize:13,fontWeight:700,cursor:"pointer"},
    resetBtn:{background:"transparent",border:"none",color:"#555",fontSize:12,cursor:"pointer",textDecoration:"underline"},
