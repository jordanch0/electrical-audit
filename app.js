...
// ─────────────────────────────────────────────────────────────────────────
function EventCard({ev, compact=false, onToggleComplete}) {
  ...
  return React.createElement('div',{style:{background:"#1a1a1a",border:`2px solid ${uc}44`,borderRadius:12,padding:"12px 14px",marginBottom:8,opacity:ev.completed?0.7:1}}
    ,React.createElement('div',{style:{display:"flex",alignItems:"flex-start",gap:10}}
      ...
      ,!compact&&!ev.completed&&React.createElement('div',{style:{display:"flex",gap:6,marginLeft:8,alignItems:"flex-start"}}
        ,React.createElement('button',{style:{...SI.smallBtn,color:"#818cf8",borderColor:"#6366f144"},onClick:()=>startEdit(ev)},"✏️ Edit")
        ,del
          ?React.createElement('div',{style:{display:"flex",flexDirection:"column",gap:4}}
            ...
            ,ev.seriesId&&React.createElement('button',{style:{...SI.smallBtn,color:"#f87171",borderColor:"#ef444455",fontSize:11},onClick:()=>{deleteSeries(ev.seriesId);setDel(false);}},"Delete all ",ev.seriesTotal," in series")
...
function UpcomingView({overdue,upcoming7,upcoming30,future,completedEvents,activeEvents,toggleComplete}){
  ...
  return React.createElement('div',{style:{padding:"16px"}}
    ...
    ,overdue.length>0&&React.createElement('div',{style:{background:"#1e0a0a",border:"2px solid #ef4444",borderRadius:14,padding:"12px 14px",marginBottom:14}}
      ...
      ,overdue.map(ev=>React.createElement(EventCard,{key:ev.id,ev,onToggleComplete:toggleComplete}))
    )
    ...
    ,upcoming7.length>0&&React.createElement('div',{style:{marginBottom:14}}
      ...
      ,upcoming7.map(ev=>React.createElement(EventCard,{key:ev.id,ev,onToggleComplete:toggleComplete}))
    )
    ...
    ,upcoming30.length>0&&React.createElement('div',{style:{marginBottom:14}}
      ...
      ,upcoming30.map(ev=>React.createElement(EventCard,{key:ev.id,ev,onToggleComplete:toggleComplete}))
    )
    ...
    ,future.length>0&&React.createElement('div',{style:{marginBottom:14}}
      ...
      ,showFuture&&future.map(ev=>React.createElement(EventCard,{key:ev.id,ev,onToggleComplete:toggleComplete}))
    )
    ...
    ,completedEvents.length>0&&React.createElement('div',{style:{marginBottom:14}}
      ...
      ,showCompleted&&completedEvents.map(ev=>React.createElement(EventCard,{key:ev.id,ev,onToggleComplete:toggleComplete}))
    )
  );
}
...
function TATApp({ onGoHome }) {
  ...
  React.useEffect(()=>{
    ...
    (async()=>{
      try{
        const[p,r,m,h]=await Promise.all([load(K_TAT_PROJECTS,[]),load(K_TAT_RESULTS,{}),load(K_TAT_META,{}),load(K_TAT_HISTORY,[])]);
        clearTimeout(t);setProjects(p);setAllResults(r);setAllMeta(m);setHistory(h);setLoaded(true);
      }catch(e){clearTimeout(t);setLoaded(true);}
    })();
  },[]);
  ...
  return React.createElement('div',{style:ST.root}
    ...
    ,React.createElement('main',{style:ST.main}
      ...
      ,view==="home"&&project&&React.createElement(TATHomeView,{project,meta,setMeta,results:allResults,summary,
        ...
        onReport:()=>setView("report"),onManage:()=>setView("manage"),onHistory:()=>setView("history"),
        onReset:()=>setAllResults(prev=>({...prev,[activeProject]:{}})),
      ...
      ,isAudit&&project&&auditEntered&&activeAreaId&&React.createElement(TATItemGrid,{area,project,results:allResults,meta,
      ...
      ,view==="manage"&&project&&React.createElement(TATManageView,{project,onUpdateProject:updated=>setProjects(prev=>prev.map(p=>p.id===updated.id?updated:p))})
    ...
    // Item detail modal
    ,detailItemId&&area&&React.createElement(TATItemModal,{
...
// T&T HOME VIEW
// ─────────────────────────────────────────────────────────────────────────
function TATHomeView({project,meta,setMeta,results,summary,onStartAudit,onReport,onManage,onHistory,onReset,onExport,onArchive,auditEntered,onCompleteAudit}){
  const[showExports,setShowExports]=React.useState(false);
  const hasAuditor=!!(meta.auditor&&meta.auditor.trim());
  ...
  return React.createElement('div',{style:ST.homeWrap}
    ...
    ,React.createElement('div',{style:{width:"100%",maxWidth:500,background:"#161616",border:`1px solid ${auditEntered?TAT_COLOR+"44":"#2a2a2a"}`,borderRadius:12,padding:"10px 14px"}}
      ...
      ,!auditEntered&&React.createElement('button',{style:{...ST.smallBtn,width:"100%",color:TAT_COLOR,borderColor:`${TAT_COLOR}44`,padding:"8px"},onClick:()=>{archiveAudit&&archiveAudit();}},"📁 Archive Audit")
    )
    ,React.createElement('div',{style:{width:"100%",maxWidth:500,display:"flex",gap:8,flexWrap:"wrap"}}
...
// T&T ITEM GRID
// ─────────────────────────────────────────────────────────────────────────
function TATItemGrid({area,project,results,meta,onPatch,onOpenDetail}){
  const s=tatAreaSummary(results,project.id,area.id,area.items);
  ...
  return React.createElement('div',{style:ST.circuitWrap}
    ...
    ,React.createElement('div',{style:{display:"flex",flexDirection:"column",gap:8}}
      ,area.items.map(itemId=>{
        ...
        const name=itemNames[itemId]||d.tag||d.desc||itemId;
        const freqLabel=TAT_FREQUENCIES.find(f=>f.value===d.freq)?.label||"3 Months";
        return React.createElement('div',{key:itemId,
          ...
          ,React.createElement('div',{style:{flex:1,padding:"12px 14px",minWidth:0}}
            ,React.createElement('div',{style:{fontSize:14,fontWeight:800,color:"#eee",marginBottom:2}},name)
            ,React.createElement('div',{style:{fontSize:11,color:"#666",marginBottom:4}},d.equipType||"",d.equipType&&d.desc?" · ":"",d.desc||"")
...
// ─────────────────────────────────────────────────────────────────────────
function TATItemModal({itemId,area,project,results,meta,onPatch,onClose}){
  const item=tatGetItem(results,project.id,area.id,itemId);
  ...
  return React.createElement('div',{style:ST.modalOverlay,onClick:onClose}
    ,React.createElement('div',{style:ST.modalBox,onClick:e=>e.stopPropagation()}
      ,React.createElement('div',{style:ST.modalHeader}
        ,React.createElement('div',null
          ,React.createElement('div',{style:{fontSize:18,fontWeight:800,color:"#eee"}},"🏷 ",name)
      ...
      ,React.createElement('div',{style:{display:"flex",gap:10,marginBottom:14}}
        ,React.createElement('div',{style:{flex:1}}
          ,React.createElement('label',{style:ST.modalLabel},"ASSET ID / TAG")
          ,React.createElement('input',{style:ST.modalInput,value:item.tag||"",placeholder:"e.g. TAG-001",onChange:e=>onPatch({tag:e.target.value})})
        )
        ,React.createElement('div',{style:{flex:2}}
          ,React.createElement('label',{style:ST.modalLabel},"DESCRIPTION")
          ,React.createElement('input',{style:ST.modalInput,value:item.desc||"",placeholder:"e.g. Angle Grinder 9\"",onChange:e=>onPatch({desc:e.target.value})})
        )
      )
      ...
      ,React.createElement('div',{style:{display:"flex",gap:10,marginBottom:14}}
        ,React.createElement('div',{style:{flex:2}}
          ...
          ,React.createElement('select',{style:ST.modalInput,value:item.equipType||"",onChange:e=>onPatch({equipType:e.target.value})}
            ...
            ,TAT_EQUIP_TYPES.map(t=>React.createElement('option',{key:t,value:t},t))
          )
        )
        ,React.createElement('div',{style:{flex:1}}
          ...
          ,React.createElement('select',{style:ST.modalInput,value:item.freq||"3",onChange:e=>onPatch({freq:e.target.value})}
            ,TAT_FREQUENCIES.map(f=>React.createElement('option',{key:f.value,value:f.value},f.label))
          )
        )
      )
...
// T&T MANAGE VIEW
// ─────────────────────────────────────────────────────────────────────────
function TATManageView({project,onUpdateProject}){
  const[expandedArea,setExpandedArea]=React.useState(null);
  const[newAreaName,setNewAreaName]=React.useState("");
  const[newItemName,setNewItemName]=React.useState({});
  const[bulkItems,setBulkItems]=React.useState({});
  ...
  const addArea=()=>{
    ...
    upd({...project,areas:[...project.areas,{id:tatSlug(newAreaName),name:newAreaName.trim(),items:[],itemNames:{}}]});
    setNewAreaName("");
  };
  const delArea=id=>upd({...project,areas:project.areas.filter(a=>a.id!==id)});
  const addItem=(areaId)=>{
    ...
    const itemId=tatUid();
    upd({...project,areas:project.areas.map(a=>a.id===areaId?{...a,items:[...a.items,itemId],itemNames:{...(a.itemNames||{}),[itemId]:n}}:a)});
    setNewItemName(x=>({...x,[areaId]:""}));
  };
  const addBulk=(areaId)=>{
    const raw=(bulkItems[areaId]||"").trim();if(!raw)return;
    const names=raw.split(",").map(s=>s.trim()).filter(Boolean);
    const newItems=names.map(n=>({id:tatUid(),name:n}));
    upd({...project,areas:project.areas.map(a=>a.id===areaId?{...a,items:[...a.items,...newItems.map(i=>i.id)],itemNames:{...(a.itemNames||{}),...Object.fromEntries(newItems.map(i=>[i.id,i.name]))}}:a)});
    setBulkItems(x=>({...x,[areaId]:""}));
  };
  const delItem=(areaId,itemId)=>upd({...project,areas:project.areas.map(a=>{if(a.id!==areaId)return a;const mn={...(a.itemNames||{})};delete mn[itemId];return{...a,items:a.items.filter(i=>i!==itemId),itemNames:mn};})});
        ...
        ,expandedArea===area.id&&React.createElement('div',{style:{padding:"8px 12px 12px"}}
          ,React.createElement('div',{style:{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8}}
            ,(area.items||[]).map(itemId=>{
              ...
              return React.createElement('div',{key:itemId,style:{display:"flex",alignItems:"center",gap:4,background:"#1e1e1e",border:`1px solid ${TAT_COLOR}33`,borderRadius:6,padding:"3px 8px"}}
                ,React.createElement('span',{style:{fontSize:12,color:"#ccc",fontWeight:600}},mn)
                ,React.createElement('button',{style:{background:"transparent",border:"none",color:"#ef4444",cursor:"pointer",fontSize:12,padding:"0 0 0 4px"},onClick:()=>delItem(area.id,itemId)},"✕")
              );
            })
            ,(area.items||[]).length===0&&React.createElement('span',{style:{fontSize:12,color:"#555"}},"No items")
          )
          ,React.createElement('div',{style:{display:"flex",gap:6,marginBottom:4}}
            ,React.createElement('input',{style:{...ST.smallInput,flex:1},placeholder:"Add item e.g. Angle Grinder 9\"",value:newItemName[area.id]||"",onChange:e=>setNewItemName(x=>({...x,[area.id]:e.target.value})),onKeyDown:e=>e.key==="Enter"&&addItem(area.id)})
            ,React.createElement('button',{style:{...ST.smallBtn,color:TAT_COLOR,borderColor:`${TAT_COLOR}55`},onClick:()=>addItem(area.id)},"+ Add")
          )
          ,React.createElement('div',{style:{fontSize:10,color:"#555",marginBottom:4}},"BULK ADD (comma-separated)")
          ,React.createElement('div',{style:{display:"flex",gap:6}}
            ,React.createElement('input',{style:{...ST.smallInput,flex:1},placeholder:"Item 1, Item 2, Item 3",value:bulkItems[area.id]||"",onChange:e=>setBulkItems(x=>({...x,[area.id]:e.target.value})),onKeyDown:e=>e.key==="Enter"&&addBulk(area.id)})
  ...
  return React.createElement('div',{style:ST.listWrap}
    ...
    ,history.map(snap=>{
      ...
      return React.createElement('div',{key:snap.id,style:{...ST.siteCard,flexDirection:"column",padding:0,marginBottom:10,overflow:"hidden"}}
        ...
        ,expanded===snap.id&&React.createElement('div',{style:{padding:"0 16px 14px",borderTop:"1px solid #2a2a2a"}}
          ,React.createElement('div',{style:{display:"flex",gap:8,marginTop:10,flexWrap:"wrap"}}
            ,React.createElement('button',{style:{...ST.smallBtn,flex:1,color:"#60a5fa",borderColor:"#3b82f644",fontWeight:700},onClick:()=>{}},"👁 View Results")
