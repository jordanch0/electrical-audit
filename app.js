...
, pnl.circuits.map(c=>{
  ...
  if(isConfirmingDelete){
    return(React.createElement('div',{key:c,style:{background:"#1e1e1e",border:"1px solid #ef444466",borderRadius:8,padding:"6px 8px",display:"flex",alignItems:"center",gap:6,width:"100%",boxSizing:"border-box"}},
      React.createElement('span',{style:{fontSize:12,color:"#ccc",fontWeight:600,flex:1}},c),
      React.createElement('span',{style:{fontSize:11,color:"#f87171",whiteSpace:"nowrap"}},"Delete?"),
      React.createElement('button',{style:{...S.smallBtn,color:"#f87171",borderColor:"#ef444466",padding:"5px 10px",fontSize:12},onClick:e=>{e.stopPropagation();delCircuit(area.id,pnl.id,c);setConfirmDeleteCircuit(null);}},"🗑 Delete"),
      React.createElement('button',{style:{...S.smallBtn,padding:"5px 10px",fontSize:12},onClick:e=>{e.stopPropagation();setConfirmDeleteCircuit(null);}},"Keep")
    ));
  }
  // Edit state
  if(isEditing){
            ...
            React.createElement('div',{key:cat.key,style:{border:`1px solid ${cat.color}33`,borderRadius:8,padding:"10px 12px",marginBottom:8,background:"#161616"}}
              ...
              ,React.createElement('div',{style:{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8}}
                ,(area.panels.find(p=>p.name===cat.key)||{circuits:[],machineNames:{}}).circuits.map(itemId=>{
                  ...
                  if(isConfirming){
                    return React.createElement('div',{key:itemId,style:{display:"flex",alignItems:"center",gap:6,background:"#1e1e1e",border:"1px solid #ef444466",borderRadius:8,padding:"6px 10px"}}
                      ,React.createElement('span',{style:{fontSize:12,color:"#ccc",fontWeight:600,flex:1}},itemName)
                      ,React.createElement('span',{style:{fontSize:11,color:"#f87171",whiteSpace:"nowrap"}},"Delete?")
                      ,React.createElement('button',{style:{...SI.smallBtn,color:"#f87171",borderColor:"#ef444466",padding:"5px 10px",fontSize:12},onClick:e=>{e.stopPropagation();delItem(area.id,cat.key,itemId);setConfirmDeleteItem(null);}},'🗑 Delete')
                      ,React.createElement('button',{style:{...SI.smallBtn,padding:"5px 10px",fontSize:12},onClick:e=>{e.stopPropagation();setConfirmDeleteItem(null);}},'Keep')
                    );
                  }
            ...
            ,(area.items||[]).map(itemId=>{
              ...
              if(isConfirming){
                return React.createElement('div',{key:itemId,style:{display:"flex",alignItems:"center",gap:6,background:"#1e1e1e",border:"1px solid #ef444466",borderRadius:8,padding:"6px 10px"}}
                  ...
                  ,React.createElement('span',{style:{fontSize:11,color:"#f87171",whiteSpace:"nowrap"}},"Delete?")
                  ,React.createElement('button',{style:{...ST.smallBtn,color:"#f87171",borderColor:"#ef444466",padding:"5px 10px",fontSize:12},onClick:e=>{e.stopPropagation();delItem(area.id,itemId);setConfirmDeleteItem(null);}},'🗑 Delete')
                  ,React.createElement('button',{style:{...ST.smallBtn,padding:"5px 10px",fontSize:12},onClick:e=>{e.stopPropagation();setConfirmDeleteItem(null);}},'Keep')
                );
              }
...
function ThermoManageView({
  ...
  project.areas.map(area => /*#__PURE__*/React.createElement("div", {
    ...
    style: {display: "flex", alignItems: "center", gap: 6, background: "#1e1e1e", border: "1px solid #ef444466", borderRadius: 8, padding: "6px 10px", marginTop: 4}
  }, /*#__PURE__*/React.createElement("span", {
    style: {fontSize: 12, color: "#ccc", fontWeight: 600, flex: 1}
  }, confirmDeleteCircuit.name), /*#__PURE__*/React.createElement("span", {
    style: {fontSize: 11, color: "#f87171", whiteSpace: "nowrap"}
  }, "Delete?"), /*#__PURE__*/React.createElement("button", {
    style: {...STH.smallBtn, color: "#f87171", borderColor: "#ef444466", padding: "5px 10px", fontSize: 11},
    ...
    style: {...STH.smallBtn, padding: "5px 10px", fontSize: 11},
