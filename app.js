...
, pnl.circuits.map(c=>{
  ...
  return(React.createElement('div', { key: c, style: {background:"#1e1e1e",border:`1px solid ${isEditing?"#60a5fa":"#333"}`,borderRadius:8,padding:isEditing?"8px":"3px 8px",marginBottom:isEditing?4:0,transition:"all 0.15s"},}
    , React.createElement('div',{style:{display:"flex",alignItems:"center",gap:4}}
      , React.createElement('span', { style: {fontSize:12,color:"#ccc",fontWeight:600},}, c)
      , !isEditing&&cm.cbType&&React.createElement('span', { style: {fontSize:10,color:"#64748b",borderLeft:"1px solid #333",paddingLeft:5,marginLeft:2},}, cm.cbType)
      , !isEditing&&cm.ampRating&&React.createElement('span', { style: {fontSize:10,color:"#64748b"},}, " · ",cm.ampRating)
      , !isEditing&&React.createElement('button', { style: {background:"transparent",border:"none",color:"#60a5fa",cursor:"pointer",fontSize:11,padding:"0 2px 0 6px",opacity:0.7}, onClick: ()=>startEditCircuit(pnl.id,c,cm), title:"Edit CB/Amp"},  "✎")
      , React.createElement('button', { style: {background:"transparent",border:"none",color:"#ef4444",cursor:"pointer",fontSize:12,padding:"0 0 0 2px",marginLeft:"auto"}, onClick: ()=>delCircuit(area.id,pnl.id,c),}, "✕")
    )
    , isEditing&&React.createElement('div',{style:{marginTop:6,display:"flex",flexDirection:"column",gap:6}}
      , React.createElement('select',{style:{...S.smallInput,fontSize:12},value:editCbType,onChange:e=>setEditCbType(e.target.value)}
        , React.createElement('option',{value:""},"CB/RCD Type")
        , cbOptions.map(o=>React.createElement('option',{key:o,value:o},o))
      )
      , React.createElement('select',{style:{...S.smallInput,fontSize:12},value:editAmpRating,onChange:e=>setEditAmpRating(e.target.value)}
        , React.createElement('option',{value:""},"Amp Rating")
        , ampOptions.map(o=>React.createElement('option',{key:o,value:o},o))
      )
      , React.createElement('div',{style:{display:"flex",gap:6}}
        , React.createElement('button',{style:{...S.smallBtn,flex:1,color:"#4ade80",borderColor:"#22c55e55",fontSize:12},onClick:()=>saveCircuitMeta(area.id,pnl.id,c)},"✓ Save")
        , React.createElement('button',{style:{...S.smallBtn,flex:1,fontSize:12},onClick:()=>setEditingCircuit(null)},"Cancel")
      )
    )
  ));
})
...
function parseTATExcel(data) {
  ...
  const areas=Object.entries(areaMap).map(([aName,itemArr])=>({
    ...
    // itemNames: "Tag — Name" format, or just name if no tag
    ...
    ,React.createElement('div',{style:ST.modalBox,onClick:e=>e.stopPropagation()}
      ...
      ,React.createElement('div',{style:{display:"flex",gap:10,marginBottom:14}}
        ,React.createElement('div',{style:{flex:2}}
          ...
          ,React.createElement('select',{style:ST.modalInput,value:effectiveItem.equipType||"",onChange:e=>onPatch({equipType:e.target.value})}
        ...
        ,React.createElement('div',{style:{flex:1}}
          ...
          ,React.createElement('select',{style:ST.modalInput,value:effectiveItem.freq||"3",onChange:e=>{onPatch({freq:e.target.value});}}
...
function TATManageView({project,onUpdateProject,equipTypes,freqOptions,tatDefaults,applianceNames}){
  ...
  const[newItemFreq,setNewItemFreq]=React.useState({}); // test frequency
  const[showNameDrop,setShowNameDrop]=React.useState({});
  const[bulkItems,setBulkItems]=React.useState({});
  const[editingProject,setEditingProject]=React.useState(false);
  const[projName,setProjName]=React.useState(project.name);

  // Auto-next tag — scans ALL itemTags across ALL areas, returns plain whole number
  const nextTag=()=>{
    const usedNums=new Set();
    project.areas.forEach(a=>{
      // Check itemTags — any numeric value (001, 1, TAG-001 etc)
      Object.values(a.itemTags||{}).forEach(t=>{
        if(t){const m=String(t).match(/(\d+)/);if(m)usedNums.add(parseInt(m[1]));}
      });
      // Check itemNames — catch "5 — Drill", "005", "005 — Drill"
      Object.values(a.itemNames||{}).forEach(n=>{
        const m=String(n).match(/^(\d+)/);if(m)usedNums.add(parseInt(m[1]));
      });
    });
    let next=1;
    while(usedNums.has(next))next++;
    return String(next); // plain integer, no padding
  };
  // Check if a tag number is already used anywhere in site
  const tagExists=(num)=>{
    const n=parseInt(num);if(isNaN(n))return false;
    for(const a of project.areas){
      for(const t of Object.values(a.itemTags||{})){
        if(parseInt(t)===n)return true;
      }
    }
    return false;
  };
  ...
  const addItem=(areaId)=>{
    ...
    // Use tag as-is, but if it's numeric and already used, bump to next available
    const numericTag=parseInt(rawTag);
    let tag=rawTag;
    if(!isNaN(numericTag)&&tagExists(rawTag)&&!newItemTag[areaId]){
      tag=nextTag(); // auto-next skips used ones
    }
    const eType=(newItemEquip[areaId]||"").trim();
    const freq=(newItemFreq[areaId]||((tatDefaults||{}).freq)||"3").trim();
    const itemId=tatUid();
  ...
  const addBulk=(areaId)=>{
    ...
    const bulkFreq=(newItemFreq[areaId]||((tatDefaults||{}).freq)||"3").trim();
    // Use nextTag() as starting point, track used numbers to avoid doubles in same batch
    ...
    const newItems=names.map(n=>{
      ...
      tagNum++;while(usedInBulk.has(tagNum))tagNum++;
      usedInBulk.add(parseInt(tag));
      const itemId=tatUid();
          ...
          ,React.createElement('div',{style:{background:"#111",border:`1px solid ${TAT_COLOR}22`,borderRadius:10,padding:"10px",marginBottom:8}}
            ...
            ,React.createElement('div',{style:{display:"flex",gap:6,marginBottom:6}}
              ...
              ,React.createElement('div',{style:{flex:2,position:"relative"}}
                ...
                ,showNameDrop[area.id]&&React.createElement('div',{
                  style:{position:"absolute",top:"100%",left:0,right:0,background:"#1e1e1e",border:`1px solid ${TAT_COLOR}55`,borderRadius:8,zIndex:50,maxHeight:180,overflowY:"auto",boxShadow:"0 4px 20px rgba(0,0,0,0.6)",marginTop:2}
                }
                  ,(applianceNames||TAT_DEFAULT_NAMES)
                    .filter(n=>!newItemName[area.id]||n.toLowerCase().includes((newItemName[area.id]||"").toLowerCase()))
                    .map(n=>React.createElement('button',{
                      key:n,
                      style:{display:"block",width:"100%",padding:"10px 14px",background:"transparent",border:"none",borderBottom:"1px solid #2a2a2a",color:"#eee",fontSize:13,textAlign:"left",cursor:"pointer"},
                      onMouseDown:e=>{e.preventDefault();setNewItemName(x=>({...x,[area.id]:n}));setShowNameDrop(x=>({...x,[area.id]:false}));}
                    },n)
                  )
            ...
            ,React.createElement('div',{style:{display:"flex",gap:6,marginBottom:8}}
              ...
              ,React.createElement('div',{style:{flex:1}}
                ...
                ,React.createElement('select',{style:{...ST.smallInput,width:"100%"},value:newItemFreq[area.id]||((tatDefaults||{}).freq)||"3",onChange:e=>setNewItemFreq(x=>({...x,[area.id]:e.target.value}))}
                  ,(freqOptions||TAT_DEFAULT_FREQS).map(f=>React.createElement('option',{key:f.value,value:f.value},f.label))
                )
              )
            )
