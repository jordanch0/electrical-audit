...
function TATApp({ onGoHome }) {
  ...
  React.useEffect(()=>{
    ...
    (async()=>{
      try{
        const[p,r,m,h,et,fq,an]=await Promise.all([load(K_TAT_PROJECTS,[]),load(K_TAT_RESULTS,{}),load(K_TAT_META,{}),load(K_TAT_HISTORY,[]),load(K_TAT_SETTINGS,TAT_DEFAULT_EQUIP_TYPES),load(K_TAT_FREQS,TAT_DEFAULT_FREQS),load(K_TAT_NAMES,TAT_DEFAULT_NAMES)]);
        clearTimeout(t);setProjects(p);setAllResults(r);setAllMeta(m);setHistory(h);setEquipTypes(et||TAT_DEFAULT_EQUIP_TYPES);setFreqOptions(fq||TAT_DEFAULT_FREQS);setApplianceNames(an||TAT_DEFAULT_NAMES);setLoaded(true);
      }catch(e){clearTimeout(t);setLoaded(true);}
    })();
  },[]);
  ...
  return React.createElement('div',{style:ST.root}
    ...
    ,React.createElement('main',{style:ST.main}
      ...
      ,view==="manage"&&project&&React.createElement(TATManageView,{project,equipTypes,applianceNames,onUpdateProject:updated=>setProjects(prev=>prev.map(p=>p.id===updated.id?updated:p))})
      ,view==="settings"&&React.createElement(TATSettingsView,{equipTypes,setEquipTypes,freqOptions,setFreqOptions,applianceNames,setApplianceNames})
...
// T&T MANAGE VIEW
// ─────────────────────────────────────────────────────────────────────────
function TATManageView({project,onUpdateProject,equipTypes,applianceNames}){
  const[expandedArea,setExpandedArea]=React.useState(null);
  const[newAreaName,setNewAreaName]=React.useState("");
...
// ─────────────────────────────────────────────────────────────────────────
function TATSettingsView({equipTypes, setEquipTypes, freqOptions, setFreqOptions, applianceNames, setApplianceNames}) {
