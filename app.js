...
function TATManageView({project,onUpdateProject,equipTypes,freqOptions,tatDefaults,applianceNames}){
  ...
  const[confirmDeleteItem,setConfirmDeleteItem]=React.useState(null); // {areaId, itemId}
          ...
          ,React.createElement('div',{style:{display:"flex",flexDirection:"column",gap:4,marginBottom:10}}
            ,(area.items||[]).map(itemId=>{
              ...
              return React.createElement('div',{key:itemId,style:{display:"flex",alignItems:"center",gap:6,background:"#1e1e1e",border:`1px solid ${TAT_COLOR}33`,borderRadius:8,padding:"6px 10px"}}
                ...
                ,React.createElement('button',{style:{background:"#2a1010",border:"1px solid #ef444433",color:"#ef4444",cursor:"pointer",fontSize:13,padding:"4px 9px",borderRadius:6,flexShrink:0},onClick:()=>setConfirmDeleteItem({areaId:area.id,itemId})},"✕")
              );
            })
