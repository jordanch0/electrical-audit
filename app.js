...
, React.createElement('button', {
      ...
      , React.createElement('button', {
          style:{width:"100%",maxWidth:420,display:"flex",alignItems:"center",gap:20,padding:"24px 24px",background:"#161616",border:"2px solid #a855f744",borderRadius:20,cursor:"pointer",color:"#eee",marginBottom:14,textAlign:"left"},
...
function exportSWBExcel(project, allResults, meta) {
  ...
  if(window.webkit&&window.webkit.messageHandlers&&window.webkit.messageHandlers.shareFile){
    ...
    const a=document.createElement("a"); a.href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,"+wbOut; a.download=fname; a.click();
  }
}
...
function parseSWBExcel(data) {
  try {
    ...
    for(let i=0;i<Math.min(rows.length,10);i++){
      const r=rows[i].map(c=>String(c).toLowerCase());
      if(r.some(c=>c.includes("area")||c.includes("board"))){hi=i;break;}
    }
    if(hi<0) hi=0;
    ...
    return {areas,siteName:"",siteId:swbSlug("imported")};
  } catch(e){ return null; }
}

function downloadSWBTemplate() {
  const wb=XLSX.utils.book_new();
  const ws=XLSX.utils.aoa_to_sheet([["Area","Board / Panel Name"],["ONR Wash Plant","MCC 1"],["ONR Wash Plant","MDB 1"],["Sub Station","MSB"]]);
  ws["!cols"]=[{wch:24},{wch:28}];
  XLSX.utils.book_append_sheet(wb,ws,"Switchboard Import");
...
function SWBApp({ onGoHome }) {
  ...
  React.useEffect(()=>{
    (async()=>{
      try{const [p,r,m,h]=await Promise.all([load(K_SWB_PROJECTS,[]),load(K_SWB_RESULTS,{}),load(K_SWB_META,{}),load(K_SWB_HISTORY,[])]);setProjects(p);setAllResults(r);setAllMeta(m);setHistory(h);}
  ...
  return React.createElement('div',{style:SS.root}
    ...
    ,view!=="projects"&&React.createElement('div',{style:SS.breadcrumb}
      ...
      ,view==="area"&&React.createElement(React.Fragment,null,React.createElement('span',{style:SS.bcSep},"›"),React.createElement('span',{style:{...SS.bcItem,color:"#a855f7"}},"Audit"))
      ,view==="board"&&board&&React.createElement(React.Fragment,null
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:"#aaa"},onClick:goArea},area&&area.name||"")
        ,React.createElement('span',{style:SS.bcSep},"›")
        ,React.createElement('span',{style:{...SS.bcItem,color:"#a855f7"}},board.name)
      )
      ,["manage","report","history"].includes(view)&&React.createElement(React.Fragment,null
        ,React.createElement('span',{style:SS.bcSep},"›")
    ...
    ,React.createElement('div',{style:SS.main}
      ,view==="projects"&&React.createElement(SWBProjectListView,{projects,allResults,onSelect:pid=>{setActiveProject(pid);setView("home");},onAddProject:p=>setProjects(prev=>[...prev,p]),onDeleteProject:pid=>{setProjects(prev=>prev.filter(p=>p.id!==pid));setAllResults(prev=>{const n={...prev};delete n[pid];return n;});setAllMeta(prev=>{const n={...prev};delete n[pid];return n;});}})
      ,view==="home"&&project&&React.createElement(SWBHomeView,{project,meta,setMeta,results:allResults,summary,onStartAudit:()=>setView("area"),onReport:()=>setView("report"),onManage:()=>setView("manage"),onHistory:()=>setView("history"),onExport:()=>exportSWBExcel(project,allResults,meta),onArchive:archiveAudit})
      ,view==="area"&&project&&React.createElement(SWBAreaListView,{project,results:allResults,onSelectBoard:(aid,bid)=>{setActiveAreaId(aid);setActiveBoardId(bid);setView("board");}})
      ,view==="board"&&board&&React.createElement(SWBBoardView,{board,area,project,results:allResults,activeItemKey,onOpenItem:key=>setActiveItemKey(key),onCloseItem:()=>setActiveItemKey(null),onPatch:(key,patch)=>patchItem(activeAreaId,activeBoardId,key,patch),onResetBoard:()=>resetBoard(activeAreaId,activeBoardId)})
      ...
      ,view==="history"&&React.createElement(SWBHistoryView,{history:history.filter(h=>h.projectId===activeProject),onDelete:id=>setHistory(prev=>prev.filter(h=>h.id!==id))})
    )
    ,view!=="projects"&&React.createElement('nav',{style:SS.bottomNav}
      ...
      ,React.createElement(SWBNavBtn,{icon:"☑",label:"Audit",  active:view==="area"||view==="board",onClick:()=>setView("area"),color:"#a855f7"})
  ...
  const handleFile=e=>{
    ...
    reader.onload=ev=>{
      try{const data=XLSX.read(ev.target.result,{type:"binary"});const parsed=parseSWBExcel(data);
        ...
        setImportPreview(parsed);setImportName(parsed.siteName||"");
...
// HOME VIEW
// ─────────────────────────────────────────────────────────────────────────
function SWBHomeView({project,meta,setMeta,results,summary,onStartAudit,onReport,onManage,onHistory,onExport,onArchive}) {
  ...
  return React.createElement('div',{style:SS.homeWrap}
    ...
    ,React.createElement('div',{style:{width:"100%",maxWidth:500,background:"#161616",border:"1px solid #2a2a2a",borderRadius:12,padding:"10px 14px"}}
      ...
      ,archiveMsg
        ...
        :React.createElement('button',{style:{...SS.ctaPrimary,background:"#a855f7",width:"100%"},onClick:()=>{onArchive();setArchiveMsg("📁 Audit archived to history");setTimeout(()=>setArchiveMsg(""),3000);}},"📁 Archive Completed Audit")
    )
    ,React.createElement('div',{style:{width:"100%",maxWidth:500,display:"flex",gap:8,flexWrap:"wrap"}}
...
// AREA LIST VIEW
// ─────────────────────────────────────────────────────────────────────────
function SWBAreaListView({project,results,onSelectBoard}) {
  const SS=swbStyles();
  return React.createElement('div',{style:SS.listWrap}
    ,React.createElement('div',{style:SS.listTitle},"Select Board")
    ,(project.areas||[]).length===0&&React.createElement('div',{style:{color:"#555",fontSize:14}},"No boards yet. Go to ⚙ Structure to add areas and boards.")
    ,(project.areas||[]).map(area=>React.createElement('div',{key:area.id}
      ,React.createElement('div',{style:{fontSize:11,color:"#555",fontWeight:700,letterSpacing:0.8,marginBottom:6,marginTop:16,paddingLeft:2}},area.name.toUpperCase())
      ,(area.boards||[]).map(b=>{
        ...
        return React.createElement('button',{key:b.id,style:{...SS.siteCard,...(bs.fail>0?{background:"#1e1010",borderColor:"#ef444455"}:isComp?{background:"#0e1e0e",borderColor:"#22c55e33"}:{})},onClick:()=>onSelectBoard(area.id,b.id)}
      ...
      ,(area.boards||[]).length===0&&React.createElement('div',{style:{fontSize:13,color:"#555",padding:"8px 4px"}},"No boards in this area.")
    ))
  );
}
...
// BOARD VIEW
// ─────────────────────────────────────────────────────────────────────────
function SWBBoardView({board,area,project,results,activeItemKey,onOpenItem,onCloseItem,onPatch,onResetBoard}) {
  const SS=swbStyles();
  ...
  return React.createElement('div',{style:{padding:"16px",position:"relative"}}
    ...
    ,activeItemKey&&React.createElement(SWBItemModal,{itemKey:activeItemKey,board,area,project,results,onPatch,onClose:onCloseItem})
  );
}

// ─────────────────────────────────────────────────────────────────────────
// ITEM MODAL — guidance + result + fail fields
// ─────────────────────────────────────────────────────────────────────────
function SWBItemModal({itemKey,board,area,project,results,onPatch,onClose}) {
  ...
  const doSave=()=>{onPatch(itemKey,{status,defectId,comment,risk,rectified,responsibility:resp,priority});onClose();};

  return React.createElement('div',{style:SS.modalOverlay,onClick:e=>{if(e.target===e.currentTarget)doSave();}}
    ,React.createElement('div',{style:SS.modalBox}
      ...
      // Fail-only fields
      ,isFail&&React.createElement(React.Fragment,null
        ,React.createElement('div',{style:{height:1,background:"#ef444422",margin:"4px 0 12px"}})
        ,React.createElement('div',{style:{fontSize:10,color:"#f87171",letterSpacing:0.8,fontWeight:700,marginBottom:10}},"DEFECT DETAILS")
        ,React.createElement('div',{style:{display:"flex",gap:10}}
          ...
          ,React.createElement('div',{style:{...SS.modalField,flex:2}}
            ,React.createElement('label',{style:SS.modalLabel},"RISK RATING")
            ,React.createElement('div',{style:{display:"flex",gap:5}}
              ,[["","None"],["L","Low"],["M","Med"],["H","High"],["U","Urgent"]].map(([r,lbl])=>
                React.createElement('button',{key:r||"none",style:{flex:1,padding:"7px 2px",borderRadius:7,fontSize:11,fontWeight:700,cursor:"pointer",border:`1.5px solid ${r?(SWB_RISK_COLORS[r]||"#555")+"88":"#333"}`,background:risk===r?(r?SWB_RISK_COLORS[r]+"33":"#2a2a2a"):"transparent",color:risk===r?(r?SWB_RISK_COLORS[r]:"#eee"):"#555"},onClick:()=>setRisk(r)},lbl)
              )
            )
          )
        )
        ,React.createElement('div',{style:SS.modalField}
          ...
          ,React.createElement('input',{style:SS.modalInput,value:rectified,placeholder:"e.g. Scheduled for Repair",onChange:e=>setRectified(e.target.value)})
        )
        ,React.createElement('div',{style:{display:"flex",gap:10}}
          ,React.createElement('div',{style:{...SS.modalField,flex:2}}
            ,React.createElement('label',{style:SS.modalLabel},"RESPONSIBILITY")
            ,React.createElement('input',{style:SS.modalInput,value:resp,placeholder:"Company / person",onChange:e=>setResp(e.target.value)})
          )
          ,React.createElement('div',{style:{...SS.modalField,flex:1}}
  ...
  return React.createElement('div',{style:SS.listWrap}
    ...
    ,editingProject
      ?React.createElement('div',{style:{...SS.addCard,marginBottom:16,border:"1px solid #a855f744"}}
        ...
        ,[["Site Name","name"],["Company","company"],["ABN","abn"],["Electrical Licence","licence"]].map(([lbl,field])=>
          React.createElement('div',{key:field,style:{marginBottom:8}}
            ,React.createElement('div',{style:SS.metaLabelText},lbl)
            ,React.createElement('input',{style:{...SS.metaInput,marginTop:4},value:project[field]||"",placeholder:lbl,onChange:e=>upd({...project,[field]:e.target.value})})
          )
        )
      ...
      :React.createElement('button',{style:{...SS.siteCard,marginBottom:16},onClick:()=>setEditingProject(true)}
        ,React.createElement('div',{style:{flex:1}}
          ,React.createElement('div',{style:SS.siteCardName},project.name)
          ,React.createElement('div',{style:SS.siteCardSub},project.company||"—",(project.abn?` · ABN: ${project.abn}`:""))
        )
    ...
    ,(project.areas||[]).map(area=>
      React.createElement('div',{key:area.id,style:{background:"#161616",border:"1px solid #a855f722",borderRadius:12,marginBottom:10,overflow:"hidden"}}
        ,React.createElement('div',{style:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 14px",cursor:"pointer"},onClick:()=>setExpandedArea(expandedArea===area.id?null:area.id)}
          ,React.createElement('div',null
            ,React.createElement('div',{style:{fontWeight:700,color:"#eee",fontSize:14}},area.name)
            ,React.createElement('div',{style:{fontSize:11,color:"#555",marginTop:2}},(area.boards||[]).length," boards")
          )
          ,React.createElement('div',{style:{display:"flex",gap:8,alignItems:"center"}}
            ,React.createElement('button',{style:{padding:"4px 10px",background:"transparent",border:"1px solid #ef444433",borderRadius:6,fontSize:11,cursor:"pointer",color:"#ef4444"},onClick:e=>{e.stopPropagation();delArea(area.id);}},"✕")
            ,React.createElement('span',{style:{fontSize:16,color:"#555"}},expandedArea===area.id?"▲":"▼")
          )
        )
        ,expandedArea===area.id&&React.createElement('div',{style:{padding:"0 14px 14px"}}
          ,(area.boards||[]).map(b=>
            React.createElement('div',{key:b.id,style:{display:"flex",alignItems:"center",gap:8,marginBottom:8,padding:"8px 10px",background:"#111",borderRadius:8}}
              ,React.createElement('span',{style:{flex:1,fontSize:13,color:"#eee"}},b.name)
              ,React.createElement('button',{style:{padding:"3px 10px",background:"transparent",border:"1px solid #ef444433",borderRadius:6,fontSize:11,cursor:"pointer",color:"#ef4444"},onClick:()=>delBoard(area.id,b.id)},"✕")
            )
          )
...
function SWBHistoryView({history,onDelete}) {
  const [expanded,setExpanded]=React.useState(null);const [deleteId,setDeleteId]=React.useState(null);
  const SS=swbStyles();const fmtD=d=>{if(!d)return"";try{return new Date(d).toLocaleDateString("en-AU",{day:"2-digit",month:"2-digit",year:"numeric"});}catch(_){return d;}};
  if(!history||history.length===0) return React.createElement('div',{style:{...SS.listWrap,color:"#555",fontSize:14}},"No archived audits yet. Tap 'Archive' on the home screen to save a snapshot.");
  return React.createElement('div',{style:SS.listWrap}
      ...
      React.createElement('div',{key:snap.id,style:{background:"#161616",border:"1px solid #2a2a2a",borderRadius:12,marginBottom:10,overflow:"hidden"}}
        ...
        ,expanded===snap.id&&React.createElement('div',{style:{padding:"0 16px 14px"}}
          ,deleteId===snap.id
            ...
            :React.createElement('button',{style:{background:"transparent",border:"1px solid #ef444433",borderRadius:6,color:"#ef4444",fontSize:11,padding:"5px 12px",cursor:"pointer"},onClick:()=>setDeleteId(snap.id)},"Delete snapshot")
        )
      )
    )
  );
