...
// XLSX — loads SheetJS from CDN dynamically, returns a Promise<XLSX>
  _xlsxPromise = new Promise((resolve, reject) => {
    ...
    s.onerror = () => {
      _xlsxPromise = null;
      reject(new Error("Failed to load SheetJS — check network connection"));
    };
...
function downloadThermoTemplate() {
  const XLSX = window.XLSX; if (!XLSX) { alert("Excel library not loaded - please reload the page"); return; }
  const headers = ["Location", "Board", "Circuit", "Date", "Page Number", "Pass / Fail", "Rectified / Scheduled", "Date Rectified / Scheduled", "Defect ID", "Responsibility", "Notes / Recommendations", "Priority (L,M,H,U)"];
  const rows = [["Site Name — Thermographic Test — Month Year", ...Array(headers.length - 1).fill("")], ["Vorick Group Asset Maintenance Pty. Ltd.  |  ABN: 44 601 045 872  |  Electrical Licence: 319114C", ...Array(headers.length - 1).fill("")], headers, ["ONR Main Wash Plant", "Wash Plant Main Switchboard", "Main switch", "12/03/2026", "1", "PASS", "", "", "", "", "", ""], ["ONR Main Wash Plant", "Wash Plant Main Switchboard", "Main switch", "12/03/2026", "2", "PASS", "", "", "", "", "", ""], ["ONR Main Wash Plant", "Wash Plant Main Switchboard", "", "12/03/2026", "3", "PASS", "", "", "", "", "Check load balancing", ""]];
  const ws = XLSX.utils.aoa_to_sheet(rows);
  ws["!cols"] = [{
    wch: 22
  }, {
...
function ThermoHomeView({
  ...
  }, auditEntered ? "COMPLETE ACTIVE AUDIT" : "ARCHIVE COMPLETED AUDIT"), /*#__PURE__*/React.createElement(CompleteAuditBtn, {
    onComplete: auditEntered ? onCompleteAudit : onArchive
  })), /*#__PURE__*/React.createElement("div", {
    style: {
      width: "100%",
      maxWidth: 500,
...
function ThermoApp({
  ...
  }, "\u2302 Menu"))), view !== "projects" && /*#__PURE__*/React.createElement("div", {
    style: S.breadcrumb
  }, /*#__PURE__*/React.createElement("span", {
    style: S.bcItem,
    onClick: goProjects
