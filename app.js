...
function TATSettingsView({equipTypes, setEquipTypes, freqOptions, setFreqOptions}) {
  ...
  const [newFreqLabel, setNewFreqLabel] = React.useState("");
  ...
  const addFreq = () => {
    ...
    if(!newFreqLabel.trim()||!months||isNaN(months)) return;
    const val = String(months);
    if((freqOptions||[]).find(f=>f.value===val)) return;
    setFreqOptions(prev=>[...(prev||[]), {value:val, label:newFreqLabel.trim()}].sort((a,b)=>parseInt(a.value)-parseInt(b.value)));
    setNewFreqLabel(""); setNewFreqMonths("");
  };
  const removeFreq = val => setFreqOptions(prev=>(prev||[]).filter(f=>f.value!==val));
  const resetFreq = () => setFreqOptions([...TAT_DEFAULT_FREQS]);

  const Section = ({title,children}) => React.createElement('div',{style:{background:"#161616",border:`1px solid ${TAT_COLOR}33`,borderRadius:14,padding:"14px",marginBottom:16}}
    ,React.createElement('div',{style:{fontSize:13,fontWeight:700,color:TAT_COLOR,marginBottom:12}},title)
    ,children
  );
  ...
  return React.createElement('div',{style:ST.listWrap}
    ...
    ,React.createElement(Section,{title:"EQUIPMENT TYPE"}
      ,React.createElement('div',{style:{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}
        ,equipTypes.map((t,i)=>React.createElement('div',{key:t,style:{display:"flex",alignItems:"center",gap:8,background:"#1a1a1a",border:`1px solid ${TAT_COLOR}33`,borderRadius:8,padding:"8px 12px"}}
    ...
    ,React.createElement(Section,{title:"TEST FREQUENCY"}
      ,React.createElement('div',{style:{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}
        ,(freqOptions||TAT_DEFAULT_FREQS).map(f=>React.createElement('div',{key:f.value,style:{display:"flex",alignItems:"center",gap:8,background:"#1a1a1a",border:`1px solid ${TAT_COLOR}33`,borderRadius:8,padding:"8px 12px"}}
          ,React.createElement('span',{style:{fontSize:13,color:"#eee",flex:1}},f.label," (",f.value," months)")
      ...
      ,React.createElement('div',{style:{display:"flex",gap:8,marginBottom:8}}
        ,React.createElement('input',{style:{...ST.smallInput,flex:2},placeholder:"Label e.g. 2 Months",value:newFreqLabel,onChange:e=>setNewFreqLabel(e.target.value)})
        ,React.createElement('input',{style:{...ST.smallInput,flex:1},placeholder:"Months",type:"number",min:"1",value:newFreqMonths,onChange:e=>setNewFreqMonths(e.target.value),onKeyDown:e=>e.key==="Enter"&&addFreq()})
        ,React.createElement('button',{style:{...ST.ctaPrimary,background:TAT_COLOR,padding:"10px 16px",fontSize:13},onClick:addFreq},"+ Add")
      )
