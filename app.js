...
function TATManageView({project,onUpdateProject,equipTypes}){
  ...
  // Auto-next tag number for an area
  const nextTag=(areaId)=>{
    const area=project.areas.find(a=>a.id===areaId);
    if(!area)return"001";
    const existing=(area.items||[]).map(id=>{
      const mn=(area.itemNames||{})[id]||"";
      const m=mn.match(/(\d+)$/);
  ...
  const addItem=(areaId)=>{
    ...
    const tag=(newItemTag[areaId]||nextTag(areaId)).trim();
  ...
  const addBulk=(areaId)=>{
    ...
    const names=raw.split(",").map(s=>s.trim()).filter(Boolean);
    let tagNum=parseInt(nextTag(areaId))||1;
          ...
          ,React.createElement('div',{style:{background:"#111",border:`1px solid ${TAT_COLOR}22`,borderRadius:10,padding:"10px",marginBottom:8}}
            ...
            ,React.createElement('div',{style:{display:"flex",gap:6,marginBottom:6}}
              ,React.createElement('div',{style:{flex:1}}
                ...
                ,React.createElement('input',{style:{...ST.smallInput,width:"100%"},placeholder:nextTag(area.id),value:newItemTag[area.id]||"",onChange:e=>setNewItemTag(x=>({...x,[area.id]:e.target.value}))})
              )
              ,React.createElement('div',{style:{flex:2}}
                ,React.createElement('div',{style:{fontSize:9,color:"#555",marginBottom:3}},"APPLIANCE NAME *")
                ,React.createElement('input',{style:{...ST.smallInput,width:"100%"},placeholder:'e.g. Angle Grinder 9"',value:newItemName[area.id]||"",onChange:e=>setNewItemName(x=>({...x,[area.id]:e.target.value})),onKeyDown:e=>e.key==="Enter"&&addItem(area.id)})
              )
            )
...
function TATSettingsView({equipTypes, setEquipTypes, freqOptions, setFreqOptions}) {
  ...
  return React.createElement('div',{style:ST.listWrap}
    ...
    ,React.createElement('div',{style:secStyle}
      ...
      ,React.createElement('div',{style:{fontSize:10,color:"#555",marginBottom:6}},"LABEL · MONTHS (number) · ADD")
      ,React.createElement('div',{style:{display:"flex",gap:6,marginBottom:8}}
        ,React.createElement('input',{style:{...ST.smallInput,flex:2},placeholder:"e.g. 2 Months",value:newFreqLabel,onChange:e=>setNewFreqLabel(e.target.value)})
        ,React.createElement('input',{style:{...ST.smallInput,flex:1},placeholder:"2",type:"number",min:"1",value:newFreqMonths,onChange:e=>setNewFreqMonths(e.target.value),onKeyDown:e=>e.key==="Enter"&&addFreq()})
        ,React.createElement('button',{style:{...ST.ctaPrimary,background:TAT_COLOR,padding:"10px 14px",fontSize:13},onClick:addFreq},"+ Add")
      )
