...
// Auto-detect company/ABN/licence from header rows
if(!company && rows[1]) {
  const parsed = parseCompanyRow(rows[1][0]);
  if(parsed.company) company = parsed.company;
  if(parsed.abn) abn = parsed.abn;
  if(parsed.licence) licence = parsed.licence;
}
// Also try row 0 if row 1 didn't yield anything
if(!company && rows[0]) {
  const parsed = parseCompanyRow(rows[0][0]);
  if(parsed.company && !parsed.company.toLowerCase().includes("area") && !parsed.company.toLowerCase().includes("panel")) {
...
function parseIELExcel(data){
  ...
  if(rows[1]&&rows[1][0]){const p=parseCompanyRow(rows[1][0]);parsedCompany=p.company||"";parsedAbn=p.abn||"";parsedLicence=p.licence||"";}
