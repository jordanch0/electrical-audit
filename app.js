...
function TATApp({ onGoHome }) {
  ...
  return React.createElement('div',{style:ST.root}
    ...
    ,React.createElement('main',{style:ST.main}
      ...
      ,isAudit&&project&&!auditEntered&&React.createElement(AuditGatePage,{color:TAT_COLOR,moduleLabel:"TEST & TAG",auditLabel:"Test & Tag Audit",hasActiveAudit:!!(meta&&meta.auditor&&meta.auditor.trim()),onGoHome:goHome,onCompleteAudit:()=>{archiveAudit();setAllResults(prev=>({...prev,[activeProject]:{}}));setAllMeta(prev=>({...prev,[activeProject]:{auditor:"",testDate:new Date().toISOString().slice(0,10),notes:""}}));setAuditEntered(false);},onEnterAudit:()=>setAuditEntered(true),isRCD:false})
