...
function SWBApp({ onGoHome }) {
  ...
  const completeAudit=()=>{
    archiveAudit();
    // Reset all results for the project
  ...
  return React.createElement('div',{style:SS.root}
    ...
    ,React.createElement('div',{style:SS.main}
      ...
      ,view==="home"&&project&&React.createElement(SWBHomeView,{project,meta,setMeta,results:allResults,summary,onStartAudit:()=>setView("area"),onReport:()=>setView("report"),onManage:()=>setView("manage"),onHistory:()=>setView("history"),onExport:()=>exportSWBExcel(project,allResults,meta),onArchive:archiveAudit,onComplete:completeAudit})
      ...
      ,view==="item"&&board&&activeItemKey&&React.createElement(SWBItemPage,{itemKey:activeItemKey,board,area,project,results:allResults,onPatch:(key,patch)=>patchItem(activeAreaId,activeBoardId,key,patch),onClose:()=>{setActiveItemKey(null);setView("board");}})
      ...
      ,view==="history"&&React.createElement(SWBHistoryView,{history:history.filter(h=>h.projectId===activeProject),project,onDelete:id=>setHistory(prev=>prev.filter(h=>h.id!==id)),onExportSnap:snap=>exportSWBExcel(project,{[project.id]:snap.results||{}},snap.meta||{}),onContinueFromSnap:snap=>{setAllResults(prev=>({...prev,[activeProject]:JSON.parse(JSON.stringify(snap.results||{}))}));setAllMeta(prev=>({...prev,[activeProject]:{...snap.meta}}));setView("home");}})
    )
    ,view!=="projects"&&React.createElement('nav',{style:SS.bottomNav}
      ...
      ,React.createElement(SWBNavBtn,{icon:"☑",label:"Audit",  active:view==="area"||view==="board"||view==="item",onClick:()=>setView("area"),color:"#a855f7"})
...
// HOME VIEW
// ─────────────────────────────────────────────────────────────────────────
function SWBHomeView({project,meta,setMeta,results,summary,onStartAudit,onReport,onManage,onHistory,onExport,onArchive,onComplete}) {
  ...
  return React.createElement('div',{style:SS.homeWrap}
    ...
    ,React.createElement('div',{style:{width:"100%",maxWidth:500,background:"#161616",border:`1px solid ${auditInProgress?"#a855f744":"#2a2a2a"}`,borderRadius:12,padding:"10px 14px"}}
      ...
      ,archiveMsg
        ...
        :auditInProgress
          ?React.createElement(SWBCompleteAuditBtn,{onComplete:()=>{onComplete();setArchiveMsg("📁 Archived — ready for new audit");setTimeout(()=>setArchiveMsg(""),3000);}})
...
// ITEM DETAIL PAGE
// ─────────────────────────────────────────────────────────────────────────
function SWBItemPage({itemKey,board,area,project,results,onPatch,onClose}) {
        ...
        ,editingAreaId===area.id
          ...
          :React.createElement(React.Fragment,null
            ...
            ,React.createElement('div',{style:{display:"flex",gap:6,alignItems:"center"}}
              ,React.createElement('button',{style:{padding:"4px 10px",background:"transparent",border:"1px solid #a855f733",borderRadius:6,fontSize:11,cursor:"pointer",color:"#a855f7"},onClick:()=>{setEditAreaName(area.name);setEditingAreaId(area.id);setConfirmDelArea(null);}},"✎")
              ,confirmDelArea===area.id
                ?React.createElement('div',{style:{display:"flex",gap:6,alignItems:"center"}}
                  ,React.createElement('span',{style:{fontSize:11,color:"#f87171"}},"Delete?")
                  ,React.createElement('button',{style:{padding:"4px 10px",background:"#3d1a1a",color:"#f87171",border:"1px solid #ef4444",borderRadius:6,fontSize:11,cursor:"pointer"},onClick:()=>{delArea(area.id);setConfirmDelArea(null);}},"Yes")
                  ,React.createElement('button',{style:{padding:"4px 8px",background:"transparent",color:"#aaa",border:"1px solid #333",borderRadius:6,fontSize:11,cursor:"pointer"},onClick:()=>setConfirmDelArea(null)},"No")
                )
                :React.createElement('button',{style:{padding:"4px 10px",background:"transparent",border:"1px solid #ef444433",borderRadius:6,fontSize:11,cursor:"pointer",color:"#ef4444"},onClick:()=>setConfirmDelArea(area.id)},"✕")
          ...
          ,editingBoard&&editingBoard.aid===area.id&&editingBoard.bid===b.id
            ...
            :confirmDelBoard&&confirmDelBoard.aid===area.id&&confirmDelBoard.bid===b.id
              ?React.createElement('div',{style:{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"#1e1010",border:"1px solid #ef444433",borderRadius:8}}
                ...
                ,React.createElement('button',{style:{padding:"5px 12px",background:"#3d1a1a",color:"#f87171",border:"1px solid #ef4444",borderRadius:6,fontSize:12,cursor:"pointer"},onClick:()=>{delBoard(area.id,b.id);setConfirmDelBoard(null);}},"Delete")
                ,React.createElement('button',{style:{padding:"5px 10px",background:"transparent",color:"#aaa",border:"1px solid #333",borderRadius:6,fontSize:12,cursor:"pointer"},onClick:()=>setConfirmDelBoard(null)},"Cancel")
              )
              :React.createElement('div',{style:{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",background:"#111",borderRadius:8}}
                ...
                ,React.createElement('button',{style:{padding:"4px 8px",background:"transparent",border:"1px solid #a855f733",borderRadius:5,fontSize:11,cursor:"pointer",color:"#a855f7"},onClick:()=>{setEditBoardName(b.name);setEditingBoard({aid:area.id,bid:b.id});setConfirmDelBoard(null);}},"✎")
                ,React.createElement('button',{style:{padding:"4px 10px",background:"transparent",border:"1px solid #ef444433",borderRadius:6,fontSize:11,cursor:"pointer",color:"#ef4444"},onClick:()=>{setConfirmDelBoard({aid:area.id,bid:b.id});setEditingBoard(null);}},"✕")
              )
        ))
