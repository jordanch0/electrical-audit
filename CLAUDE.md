...
- **Print-friendly exports (2026-09-26)** — RCD (push + injection), IEL, TAT, Thermo, IRT and ELT are split into a NARROW main results table (`#`, identifiers, dates, result, key values, **Notes** — compliance dates and pass-row notes never move) plus a separate **`Defects` sheet** (FAIL rows only: Defect ID, Priority, Rectified / Scheduled, Date Rectified / Scheduled, Responsibility, Notes) linked by the `#` column, which is a plain row number assigned at export (Defect ID is optional / not unique, so it cannot be the key). The Defects sheet ALWAYS exists with its headings, plus a "No defects recorded" line when empty. IRT has THREE sheets: `Register` (short + narrow — the client-PDF sheet), `Readings` (all 10 readings, units stated once in the header block) and `Defects`; the Register stays the FIRST sheet because every importer reads sheet 1 by heading keywords. RCD's old Summary "Failed Circuits" list is replaced by `Defects` (a >300 ms injection result now counts as a defect there too). Builders: `xlSplitSheets` / `xlSheet` (SheetJS) — headings must fit their column width (SheetJS cannot wrap: `export-print-layout.test.js` enforces it). **Page setup is in the file itself**: SheetJS silently drops orientation / fit-to-width / repeating headings, so the five SheetJS exports are post-processed by `xlPrintify` (JSZip): A4, landscape (Summary portrait), fit to 1 page wide, 0.25" side margins, printed gridlines, "Page x of y" footer, heading row 5 repeated. ELT (ExcelJS) sets the same natively. The exports are therefore `async`. To eyeball the layout in Excel / Sheets: `GEN_OUT=./sample-exports npx vitest run src/sample-exports.test.js` writes 60-row samples of every affected module.
